    ...
    for p in p_range: #Loop over x, where x=10^p, so we get more points around prob's closer to 1 and can see phase transition-y behavior more clearly.
        ...
        for _ in range(int(number_of_trials)):
            ...
            if length_of_longest_path > 2:
                ms_dimension = ms.mpsd(DAG_with_removal, longest_path)[0]
            else:
                ms_dimension = 0
                ms_failed_trials += 1
...
plot_lp_with_removal(D_selection =[1], number_of_trials = 10, number_of_probabilities_considered = 5 , N=10, data_filename = './mm_with_errors_testing.txt', savefig_filename = 'mm_with_errors_testing.pdf')
