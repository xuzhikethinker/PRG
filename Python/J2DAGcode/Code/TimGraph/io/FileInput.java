...
public class FileInput {
     ...
     TextReader data;     // Character input stream for reading noNamedata.
...
//     int maxVertexLabel=-minVertexLabel;
 ...
 // ***************************************************************************
    ...
    public int processAdjacencyMatrixOld(timgraph tg)
    {
        ...
        /**
         * Reads in table of weights.
         * <p>Each line has entries separated by tabs.  No column or row headers.
         * Sets up new graph fom this noNamedata.
         * @param tg graph to be set up from its adjacency matrix file
         * @deprecated use processAdjacencyMatrix
         */
    ...
    public int processAdjacencyMatrixTabSeparated(timgraph tg, boolean headerOn, boolean rowLabelOn, boolean trimLabelWhiteSpaceOn)
    {
       return processAdjacencyMatrix( tg, '\t', headerOn, rowLabelOn, trimLabelWhiteSpaceOn);
         ...
         * @param headerOn first row is a label for the vertices
         * @param rowLabelOn first column is a label for the rows
    ...
    public int processAdjacencyMatrixOld(timgraph tg, String wordSeparator, boolean headerOn, boolean rowLabelOn, boolean trimLabelWhiteSpaceOn)
    {
      ...
      // ***************************************************************************
         ...
         * The weights must be in the column specified by <tt>columnWeigh</tt> 
         * unless this is negative in which case no weights are read.
         * Note that columns are counted with first column being word one.
   ...
   // ***************************************************************************
         ...
         * The weights must be in the column specified by <tt>columnWeigh</tt>
         * unless this is negative in which case no weights are read.
         * The labels of edges must be in the column specified by <tt>columnLabel</tt>
         ...
         * @param ext extension of filename name of file to read noNamedata
         * @param columnSource column with source, counting from 1
...
// ***************************************************************************
         ...
         * equivalent integers.
         * @param ext extension of filename name of file to read noNamedata
         * @param columnSource column with source, first column is word 1
         * @param columnTarget column with target, counting from 1
         * @param columnWeight column with weight, counting from 1.  If negative no weights read
         * @param columnLabel column with edge label, counting from 1.  If negative no edge labels read.
...
//            String target ="NOTSET";
   ...
   // ***************************************************************************
         ...
         * <p>Each line has entries separated by whitespace.
         * The source and target vertices must be in columns specified by arguments.
         * The weights must be in the column specified by <tt>columnWeight</tt>
         * unless this is negative in which case no weights are read.
         * Note that columns are counted with first column being word one.
         * Directed, weighted and labelled nature set by input parameters.
            ...
            boolean directed, boolean vertexLabelled )
    {
        ...
        for (int e=0; e < edgeLL.size(); e++)
        {
            ...
            if (tg.isWeighted()) {
                ...
                if (tg.isMultiEdge() || !tg.edgeExists(source,target) ) tg.addEdge(source,target,new EdgeValue(label, weight));
                  ...
                  if (res>-maxWarning && (infoLevel>-2)) System.out.println("!!! edge "+(e/2)+" is second edge from "+source+"("+edgeLL.get(e-1)+") to "+target+"("+edgeLL.get(e-1)+") -> ignored");
         ...
         */
    public int readStringEdgeFile(String fullFileName,
            int columnSource, int columnTarget, int columnWeight, int columnLabel,
...
//        tg.minimumCoordinate = new Coordinate();
         ...
         * @param columnRead column to be read in
    ...
    // ***************************************************************************
         ...
         * @param columnLabel column with label, counting from 1.  
         * @param indexL list of indices
         * @param labelL list of labels.
         * @param headerOn First line ignored if true.
         */
    public int readStringIndexLabelList(String fullfilename, int columnIndex, int columnLabel, 
            ArrayList<String> indexL, ArrayList<String> labelL, boolean headerOn)
    {
         ...
         * @param columnIndex column with index, counting from 1.  If less than 1, (line word -1) is used for index if needed.
         * @param columnLabel column with label, counting from 1.
    ...
    {            
         ...
         * @param firstColumnIndex true if first column is the index
         * @param headerOn First line ignored if true.
         * @param infoLevel 2 or more gives some info on each line, 3 or more gives even more
         */
...
//        tg.minimumCoordinate = new Coordinate();
    ...
    /**
      * Reads in list of source vertices as string labels from first column of a file.
      * <p>Each line has entries separated by whitespace.
      * First entry is the vertex of interest, rest are ignored.
      * <p> No check for a bipartite graph and no lines are skipped.
      * @param fullFileName used to determine directory and name of file.
      * @return ordered set of vertices found in first column
      */
