...
public class DistributionAnalysis {
            // Input parameters
            String WAVersion="DistributionAnalysis041018";
            String nameroot;
            String dirname;
            String inputddext;
    ...
    public DistributionAnalysis() {
                dirname= "/PRG/networks/DistributionAnalysis/output/";
                nameroot = "test";
                inputddext = ".Jdd.txt";
...
//            totdd2arr = null;
    ...
    public static void main(String[] args) {
        ...
        wa.processDistanceAll("#" ,  "diststats"+wa.inputddext);
...
// ----------------------------------------------------------------------
    /**
     *  Method of DistributionAnalysis
     * This sets up the log binning. From kminbin to kmaxbin, it breaks
  ...
  /**
    ...
    void FileOutputDegreeDistribution(String filenameroot, String cc)  {
        ...
        try {
            ...
            double nlast = 0;
            double klast = 0;
            PS.println(cc+" No.vertices:\t"+ Nvertices+" No.edges:\t "+ Nedges +"\t "+WAVersion+"\t "+date);
  ...
  /**
    ...
    void FileOutputDD(String cc, String filenameroot)  
    {
        ...
        try {
            fout = new FileOutputStream(filename);
            PS = new PrintStream(fout);
            System.out.println(cc+" writing data to "+filename);
            Date date = new Date();
            double n=0;
            double nlast = 0;
            double klast = 0;
    ...
    void FileOutputLogBinDD(String filename, String cc)
    {
       // next bit of code p327 Schildt and p550
        ...
        try {
            ...
            PS = new PrintStream(fout);
            System.out.println(cc+" writing log bin data to "+filenamefull);
            Date date = new Date();
            PS.println(cc+" Log Binned Degree Distribution ratio\t "+lbratio+"\t "+WAVersion+"\t "+date);
            PS.println(cc+" N=\t "+Nvertices+"\t E= \t "+Nedges);
            PS.println(cc+" k   \t   n(k)   \t   +/-   \t bin size  \t gamma(k)");
...
// ----------------------------------------------------------------------
     ...
     * @param cc comment characters put at the start of every line
     */
    void FileOutputTotDD(String filename, String cc)
                            {
       // next bit of code p327 Schildt and p550
        PrintStream PS;
        FileOutputStream fout;
        ...
        try {
            fout = new FileOutputStream(filenamefull);
            PS = new PrintStream(fout);
            System.out.println(cc+" writing total DD data to \t"+filename);
            Date date = new Date();
            PS.println(cc+" Total Degree Distribution for \t"+nruns+"\t runs of \t"+filename+"\t "+WAVersion+"\t "+date+" ");
            PS.println(cc+"k\t   <n(k)>   \t   +/- \t gamma(k)");
            double n, err, gamma;
            double nlast = 0;
            double klast = 0;
            ...
            try{fout.close();
                } catch (IOException e) { System.out.println("File Close Error "+filename);}
            } catch (FileNotFoundException e) {
            System.out.println("Error opening output file "+filename);
        } //eo catch
    ...
    void FileOutputDistanceStats(String filename, String cc)
       ...
       // next bit of code p327 Schildt and p550
        ...
        try {
            ...
            System.out.println(cc+" writing total DD data to \t"+filename);
            Date date = new Date();
            PS.println(cc+" Total Distance stats for \t"+diameter.number+"\t runs of \t"+filename+"\t "+WAVersion+"\t "+date+" ");
 ...
 */
    public void getFileList(String ext) {
        ...
        System.out.println("Found  "+filelist.length+" files with extension "+ext);
        filenamerootlist = new String[filelist.length];
        for (int i =0; i<filelist.length; i++)
        {   filenamerootlist[i] =  getFileNameRoot(filelist[i], ext);
            System.out.println(filelist[i]+"\t "+filenamerootlist[i]);
 ...
 */
  ...
  // -----------------------------------------------------------------------       
     ...
     */
    void getDegreeDistribution(IntArrayList intddarr)  {
       kmin=99999999;
       kcont=99999999;
       kmax=-1;
//       Nvertices=0;
     ...
     * @param intddarr IntegerArrayList of degree distribution 
     */
    void getDegreeDistribution(DoubleArrayList doubleddarr)  {
       kmin=99999999;
       kcont=99999999;
       kmax=-1;
//       Nvertices=0;
    ...
    // ----------------------------------------------------------------------
      ...
      System.out.println("Starting to Degree Distribution data from " + fullfilename);
...
//      double[] number = new double[1000];  // An array to hold all
 ...
 //             System.out.println("Read " + k+"\t "+nk);
          ...
          System.out.println("Finished " + filename+"\t kmin,kcont,kmax = \t"+kmin+"\t "+kcont+"\t "+kmax);
      }//eo try
...
//              if (nk>0) System.out.println("x= " + k+"\t n(x)="+nk+"\t col="+(colno-1));
    ...
    // ----------------------------------------------------------------------
        ...
        {
          String fullfilename = dirname+filename;
          System.out.println("Starting to get Distance Statistics from " + fullfilename);
          TextReader data;     // Character input stream for reading data.
          int res=0;  // error code.
...
// **************************************************************************    
    ...
    public int ParamParse(String[] ArgList){
                for (int i=0;i< ArgList.length ;i++){
                    System.out.println("Parameter "+i+" is "+ArgList[i]);
...
// **************************************************************
     ...
     * @param String extdiststats extension for distance statistics files 
     ...
     void processDistanceAll(String cc, String extdiststats)  {
        ...
        System.out.println("Extremes of diameter: \t"+ diameterbigmin + " \t "+diameterbigmin);
...
// **************************************************************
     ...
     * @param String extddlb extension for log binned degree distribution output files
      */
     void processOne(String cc, String extddlb)  
     {
         int lbn = (int) ((lbratio-1.0)*1000.0+0.5);
         String normLabel = (NormaliseDD)?"norm":"";
            calcNumbers(ddarr);
