...
public class TimeSeries {
    ...
    /** Creates a new instance of MakeTimeSeries */
    public TimeSeries() {
...
//        ts.labelLine="gallery";
        ...
        TimeSeries tsdiff = new TimeSeries();
        if (ts.ParamParse(args)>0) return;
...
// *************************************************************************
    ...
    public int doAvalanche(String filenameroot, String cc,  DoubleArrayList timeValues)
    {
        ...
        avsizeLB.calcLogBinAny(avalancheSize.eventFreq, 0, avalancheSize.eventFreq.size()-1, 1); 
        fulltimeseriesfilename = filenameroot + avBinScale+"avsizeLB.dat";
        avsizeLB.FileOutputLogBinFreqData(fulltimeseriesfilename,cc); 
        }
        ...
        avlengthLB.calcLogBinAny(avalancheLength.eventFreq, 0, avalancheLength.eventFreq.size()-1, 1); 
        fulltimeseriesfilename = filenameroot + avBinScale+"avlengthLB.dat";
        avlengthLB.FileOutputLogBinFreqData(fulltimeseriesfilename,cc); 
        }
...
//            PS.println(cc+" Sorted time series" + SEP + TSVersion+ SEP + " "+date);
            ...
            PS.println(cc+"rank r"+ SEP + "  t   "+SEP+cc+" Sorted time series" + SEP + " No. Events ="+ timeSeries.size() + SEP + TSVersion+ SEP + " "+date);
    ...
    void FileOutputTimeSeriesFreq(String fullfilename, String cc, boolean skipZeros)
    {
       // next bit of code p327 Schildt and p550
        ...
        try {
            ...
            // Calc totals from binned data
            PS.println(cc+"Bin No."+ SEP + " bin t_min "+ SEP + " bin t_max "+SEP+" Bin t " + SEP + " Normalised Freq " + SEP +"+/-"+SEP+timeSeriesBinFreq[0].labelString(SEP) + SEP+ " No. Events ="+ timeSeries.size()+ SEP+ " No. Bins ="+ timeSeriesBinFreq.length + SEP + TSVersion+ SEP + " "+date);
