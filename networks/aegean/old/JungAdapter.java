 ...
 * @deprecated Use islandNetworks.jungInterfaces.JungCOnverter
 */
class JungAdapter {

        final static String JAVERSION = "JA060825";  // 
...
//        private double edgeMaximumThreshold; // display edges above this as of this weight
    ...
    public JungAdapter(IslandEdgeSet es, IslandSiteSet sitesInput) {
        ...
        int numSites = sites.getNumberSites();

        for(int i=0;i<numSites;i++) {
            Vertex v = vg.create();

            vID.setNumber(v, i);
            vSize.setNumber(v, sites.getDisplaySize(i));
            xCoord.setNumber(v, sites.getX(i));
            yCoord.setNumber(v, sites.getY(i));

            try{
                g.addVertex(v);
                sl.setLabel(v,sites.getName(i));
        ...
        for(int i=0;i<numSites;i++) {
            for(int j=0;j<numSites;j++) {
                ...
                vEdgeWeight.setNumber(e,es.getEdgeValue(i,j)*sites.getWeight(i));
     ...
     * Remove edges from graph which have a weight below <code>threshold</code>.
         * Use global edgeMinimumThreshold to set threshold for display of edges
     */
    public void cleanGraph() {
        ...
        while(it.hasNext()) {
            ...
            if(currentNumber < edgeZeroThreshold)
...
//                        int numberSiteVariables = sites[0].numberVariables;
