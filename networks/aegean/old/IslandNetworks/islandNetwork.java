...
public class islandNetwork {    
        ...
        final static String VERSION = "060427b";
...
/**
     ...
     */
     public void showColourValues(String cc, PrintStream PS, int dec) 
     {
        ...
        PS.println();       
        PS.print("S.Col.");
        for (int i =0; i<numberSites; i++) {PS.print(SepString+siteArray[i].displaySize);}; 
        PS.print("\n\nFrom/to");
        for (int i =0; i<numberSites; i++) {PS.print(SepString+siteArray[i].name);}; 
        PS.println();       
        for (int i =0; i<numberSites; i++) {        
...
// -------------------------------------------------------------------------
    /** Reads in site values and position from <inputnameroot>_input.dat.
    ...
    public int getSiteData() 
    {
      ...
      try {
          ...
          { // start reading new line
            ...
            // Initialise if this is the first data line  
              ...
              { // initialise if reading first proper line
                  numberSites= numberCt;
                  for (int s=0; s<numberSites; s++) siteArray[s]= new Site();
              }
          ...
          switch (tabletype)
          {
              case 0: System.out.println("No table found"); break;
              case 1: System.out.println("Distances read"); break;
              case 2: System.out.println("Penalties read"); break;
     ...
     */
    public int getSiteDistanceDataDeprecated() {
      ...
      try {
          ...
          {  // Read until end-of-line.
             siteArray[numberCt]= new Site();
             siteArray[numberCt].setName(data.getWord());
             if (infolevel>0) System.out.println("siteArray.name["+numberCt+"] = "+siteArray[numberCt].name);                     
             numberCt++;
          }
          numberSites=numberCt;

          //read in second line site sizes
          tempstring = data.getWord(); //ignore first item, labels row
          if (infolevel>0) System.out.println("next word is "+tempstring);
          numberCt = 0;
          while (data.eoln() == false) 
          {  // Read until end-of-file.
             siteArray[numberCt].size = data.getDouble();
             if (infolevel>0) System.out.println("siteArray.size["+numberCt+"] = "+siteArray[numberCt].size);                     
             numberCt++;
          }
          ...
          while (data.eoln() == false) 
          {  // Read until end-of-file.
             siteArray[numberCt].X = data.getDouble();
             if (numberCt<1) { maxX=siteArray[numberCt].X; minX=siteArray[numberCt].X;}
             else { 
                  if (siteArray[numberCt].X>maxX) maxX=siteArray[numberCt].X;
                  if (siteArray[numberCt].X<minX) minX=siteArray[numberCt].X;
                  }   
             if (infolevel>0) System.out.println("siteArray.X["+numberCt+"] = "+siteArray[numberCt].X);                     
             numberCt++;
          }
          ...
          while (data.eoln() == false) 
          {  // Read until end-of-file.
             siteArray[numberCt].Y = data.getDouble();
             if (numberCt<1) { maxY=siteArray[numberCt].Y; minY=siteArray[numberCt].Y;}
             else { 
                  if (siteArray[numberCt].Y>maxY) maxY=siteArray[numberCt].Y;
                  if (siteArray[numberCt].Y<minY) minY=siteArray[numberCt].Y;
             }
             if (infolevel>0) System.out.println("siteArray.Y["+numberCt+"] = "+siteArray[numberCt].Y);                     
             numberCt++;
          }
...
// -------------------------------------------------------------------
  /**
     ...
     * Writes site values, positions and distance data to file <inputnameroot>OUT_svpdist.dat.
     * First line site names
     * Second line site Values (fixed sizes)
     * Third line site X positions
     * Fourth line site Y positions
...
// *********************************************************************************
         ...
         */
    public void setHamiltonianParameters(double j, double mu, double b, double a, double k,  double g, double l, double ds, double ss) {
             edgeSourceH = mu;
             vertexSourceH = j;
             betaH = b;
             alphaH = a;
    ...
    public class Site
    {
          ...
          public int setInt(int index, int value)
        {
              ...
              switch (index)
              {
                  ...
                  case 14: displaySize=value; break; 
          ...
          public String toString(int index)
        {
            ...
            switch(index)
            {
                ...
                case 14: s=Integer.toString(displaySize); break;
          ...
          public int setValue(int index, String s)
        {
              ...
              try{
                  switch(index) {
                      ...
                      case 14: displaySize=new Integer(s); break;
...
// ######################################################################    
 ...
 // *******************************************************************************    
    ...
    {
        siteDistanceStats = new StatisticalQuantity();
        doDijkstra();
        for (int i=0; i<numberSites; i++) 
        {
            for (int j=0; j<numberSites; j++)
...
//      maxvedgeweightindex = 0;  
      ...
      double vV,eV;
      siteWeightStats = new StatisticalQuantity();
      edgeWeightStats = new StatisticalQuantity();     
      siteStrengthStats = new StatisticalQuantity();     
        for (int i=0; i<numberSites; i++)
        {
...
//      int zeroColourNumber =(int)(numberColours*zeroColourFrac);
      ...
      if (DisplayMaxEdgeScale>0) amew =  DisplayMaxEdgeScale;
        ...
        for (int i=0; i<numberSites; i++)
        {
           ...
           {
               case 1: siteArray[i].displaySize = (int) (0.4999 + siteArray[i].ranking*numberColours/mvw); 
                   break;
               case 2:
               case 0:
               default: siteArray[i].displaySize = (int) (0.4999 + siteArray[i].weight*numberColours/mvw); // siteArray[i].name has weight vw
           }            
  ...
  /**
    ...
    void FileOutputNetwork(String cc, int siteWeightFactor, int edgeWidthFactor, 
        ...
        try {
            ...
            for (int i=0; i<numberSites; i++)
            { 
              size = (siteWeightFactor*siteArray[i].displaySize)/numberColours;
              ...
              switch (fileType) 
              {
                  ...
                  case 1: PS.print( siteArray[i].Z
                          +" s_size 1 " 
                          + " x_fact "+siteArray[i].displaySize 
                          + " y_fact "+siteArray[i].displaySize);
                          break;
                  case 0: 
                  default: PS.print( " 0.5 "); 
              }
              int sc=siteArray[i].displaySize;
...
// -------------------------------------------------------------------
  /**
     ...
     *  <nameroot><variableName>.<ext>
     * where .clu are integers (partitions) and .vec are doubles (vectors)
     * @param variable number slects which entry to use from site
     * 
     */
        ...
        switch(variableNumber)
            {
                ...
                case 14: ext=".clu"; break; //int displaySize=-1; //14
     ...
     private class NetworkPicture extends JPanel {
         ...
         {   
            ...
            //if (islnet.infolevel>2) System.out.println("\n numberColours = "+islnet.numberColours+", edgeWidthFactor="+islnet.edgeWidthFactor);
             ...
             for (int i=0; i<islnet.numberSites; i++)
             { 
              double fracsize= (((double) islnet.siteArray[i].displaySize)/islnet.numberColours);   
            ...
            // find values for sites
             ...
             for (int iii=0; iii<numberSites; iii++)
             {
                 ...
                 switch (islnet.siteWindowMode)
                 {
                  ...
                  int ec = islnet.siteArray[i].displaySize;
//                  width = (islnet.edgeWidthFactor*ec)/islnet.numberColours;
                  if (width<1) width=1;
//                  BasicStroke bstroke = new BasicStroke((float) width);
//                  g.setStroke(bstroke);
...
//        Box infoBox = Box.createHorizontalBox();
        ...
        inputBox.add(new JLabel("             lambda "+islnet.lambdaH));
        inputBox.add(new JLabel("     distance scale "+islnet.distScaleH));
        inputBox.add(new JLabel("     short distance "+islnet.shortDistScaleH));
        inputBox.add(new JLabel("       metric number "+islnet.shortDistScaleH));        
        inputBox.add(new JLabel("               beta "+islnet.betaH));
        inputBox.add(new JLabel("   Zero Colour Frac "+islnet.zeroColourFrac));
        inputBox.add(new JLabel("   Min. Colour Frac "+islnet.minColourFrac));
 ...
 */
    ...
    JTextField  fileOutputValue, runnameValue,
               muValue,jValue,kappaValue,lambdaValue,distScaleValue,reldistScaleValue,alphaValue,betaValue,gammaValue,
    ...
    JLabel fileInputValue, fileInputMessage, fileOutputMessage, runnameMessage,
            muMessage,jMessage,kappaMessage,lambdaMessage,distScaleMessage,reldistScaleMessage,alphaMessage,betaMessage,gammaMessage,
        ...
        try
        {
            ...
            if (command.equals("READ")) {
                ...
                if (res<0) {
                    ...
                    if (jiggleScale>0.0) jiggleSiteData(distScaleH);
                    //drawInputWindow();
                }
    ...
    {   
        ...
        // edge/vertex terms now
            for (int j=0; j<numberSites; j++) if (i !=j)  
            {
               ...
               // change is site i as the from site, j as the to site
               if ( mMN0  ) dH+= 
                            - dw 
                            * ( mMN1 ? siteArray[j].getWeight() : 1 )
                            * edgePotential1(distValue[i][j])* edgeValue[i][j] ;
               // now site i is to and j is from
                if ( mMN1  ) dH+= -  dw 
...
//    };
    ...
    {  
        ...
        if (metricNumber==2) s="inverse edge strength";
        return(s) ;
    };
