...
class NetworkPicture extends JPanel {
	...
	Color[] javaColour;
		...
		numberSites = in.numberSites;
                siteSet = new IslandSiteSet; 
                for (int i=0; i< numberSites; i++)
                {
                    siteSet[i] = new IslandSite(in.siteSet[i]);
                }
		...
		displayVertexType = in.DisplayVertexType.getValue();
         ...
         *@param xsize horizontal (x) size of window
         *@param ysize vertical (y) size of window
         *@param sites array of IslandSite conatining site data (siteSet[])
	...
	{   
		...
		{ 
			sitePositionX[i] = (int)((siteSet[i].X-minX)*windowScale);
			sitePositionY[i] = (int)((siteSet[i].Y-minY)*windowScale);
		}
		...
		for (int i=0; i<numSites; i++)
		{
			if (!updateMode.isPPA())  vw=siteSet[i].getWeight();
			else vw=1;
		...
		for (int i=0; i<numSites; i++)
		{ 
			if (infolevel>2) System.out.print(Double.toString(siteSet[i].displaySize) + ", ");
			double fracsize=  siteSet[i].displaySize/numberColours ;  
			...
			{
			case 2: 
				int vc = numberColours-siteSet[siteSet[i].influence].influenceRank;
				if (vc<0) vc=0;
			...
			{
			case 2: 
				siteLabel=siteSet[siteSet[i].influence].shortName;
				break;
			case 0:
			case 1:    
			default: 
				siteLabel=siteSet[i].shortName;
			}              
