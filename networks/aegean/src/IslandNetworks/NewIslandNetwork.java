/*
 * islandNetwork.java
 * New template to be extended by all models
...
public class NewIslandNetwork {    
        ...
        protected int numberSites = MAXSITENUMBER;
...
//        public EdgeTypeSelection DisplayEdgeType; //
        ...
        /**
         * Needed to extend this basic model
         * TODO check this
         */
        public NewIslandNetwork(){}
    
    /** 
     * Creates a new instance of network.
     *@param ns number of Sites
     */
    public NewIslandNetwork(int ns) {
            inputGUI = true;
...
//             DijkstraMaxDist = MAXSEPARATION;
    ...
    */
    public NewIslandNetwork(NewIslandNetwork islnetinput) {
        //rnd = new Random(); // Give long integer arg else uses time to set seed
     ...
     */
    public NewIslandNetwork(File ifl, NewIslandNetwork inparam) 
    {
        //rnd = new Random(); // Give long integer arg else uses time to set seed
 ...
 *@param islnetinput is an inout islandNetwork to be deep copied to new one.
    */
    public void copyGeneralParameters(NewIslandNetwork islnetinput)
    {        
 ...
 *@param islnetinput is an islandNetwork to be deep copied to new one.
    */
    public void copyDisplayParameters(NewIslandNetwork islnetinput)
    {
...
//                else  autoSetOutputFileName=false;
         ...
         */
            public void Usage(){
                ...
                NewIslandNetwork i = new NewIslandNetwork(1);
                System.out.println("...............................................................................");
                System.out.println("Ariadne, iNVERSION "+NewIslandNetwork.iNVERSION+" usage: ");
    ...
    {   setOutputFileName();       
            ...
            // Output Files
              if (outputMode.isSet(OutputMode.GraphMLNetworkFileIndex)) {
                ...
                FileOutputGraphMLNetwork(NewIslandNetwork.COMMENT, maxEdgeWidthSize, directed, colourOn);
              }
//            if (outputMode.isSet(OutputMode.KMLIndex)) FileOutputKMLNetwork( "#" , 5);
              ...
              if (outputMode.isSet(OutputMode.PajekIndex)) {
                ...
                int maxEdgeWidthSize=10;
                boolean colourOn=true;
                FileOutputPajek(NewIslandNetwork.COMMENT, maxEdgeWidthSize, colourOn) ;
              }
...
// -------------------------------------------------------------------
    ...
    public void FileOutputBareNetwork(String cc, String sep){
        ...
        try {
            fout = new FileOutputStream(filenamecomplete);
            PS = new PrintStream(fout);
            if (cc.length()>0) PS.println(cc+NewIslandNetwork.iNVERSION+sep+numberSites);
...
// ***********************************************************************    
     ...
     * @deprecate call doEdgeModel() directly
     */
    public void calculateEdgeModel(){
        throw new RuntimeException("calculateEdgeModel must be overridden by class extending NewIslandNetwork");
...
//                    } // eo switch
     ...
     * appropriate <tt>do</tt><em>Name</em> routine which will define the edges.
     * The <tt>updateMode</tt> object used to choose which mode is used.
     * TODO override in each model extension
     * @param PS a print stream for the output such as System.out
...
//                        case 7: inet.doXTent(); break;
     ...
     * Should be overridden in each model 
     */
