...
public class SimulatedAnnealingvertexPartition extends VertexPartition {
    ...
    /**
     * Negative integer used to give temporary community with just one site.
     */
//    private static int TEMPCOMMUNITYLABEL =-97531;  
    ...
    initialise(tg,0,(tg.isDirected()?4:0),1,1.0,numberCommunities);
     ...
     */
    public SimulatedAnnealingvertexPartition(timgraph tg, int qdef, int nullModel,
            int qualityType, int newinfolevel, 
            int numberCommunities, int maxNumberCommunities, double lambda){
    initialise(tg,qdef, qualityType, nullModel,  lambda ,numberCommunities);
     ...
     */
    public SimulatedAnnealingvertexPartition(timgraph tg, int qdef, int nullModel, 
            int qualityType, double lambda, int newinfolevel, 
            int numberCommunities){
    initialise(tg,qdef, nullModel, qualityType, lambda,numberCommunities);
    name=SIMANNAME+quality.QdefinitionShortString();
    if (numberCommunities<2) maxCommunities=numberElements;
    else maxCommunities=numberCommunities;
    infoLevel=newinfolevel;
    ...
    private void calculateRecursively(int l, int numberOfSweeps, double betaInitial, boolean greedyAfterOneSweep) {
        ...
        if (graph.getNumberVertices() > getNumberOfCommunities()) {
            ...
            SimulatedAnnealingvertexPartition projVP = new SimulatedAnnealingvertexPartition(projg, quality.Qdefinition, quality.nullModelSwitch, quality.getQualityTypeNumber(), quality.lambda,
                    infoLevel, -1);
