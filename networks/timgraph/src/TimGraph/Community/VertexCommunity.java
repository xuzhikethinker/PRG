...
public VertexCommunity(String inputname){
    ...
    public VertexCommunity (timgraph tg, Partition p, int qdef, int qualityType, double newlambda){
        ...
        setDefaultNames();
        initialiseGraph(tg, qdef, nullModel, qualityType, newlambda);
        ...
        else throw new RuntimeException("*** Community constructor: Neither number of vertices "
                +tg.getNumberVertices()+" nor the number of edges "
                +tg.getNumberStubs()/2+" matches number of elements in partition "
                +p.getNumberElements());
    }
    /**
     * Constructs a vertex community from an edge or vertex partition.
     ...
     * @see TimGraph.Community.Quality#nullModelSwitch
     */
    public VertexCommunity (timgraph tg, Partition p, int qdef, int nullModel, int qualityType, double newlambda){
        setDefaultNames();
        initialiseGraph(tg, qdef, nullModel, qualityType, newlambda);
        if (tg.getNumberVertices()==p.getNumberElements()) initialiseFromVertexPartition(p);
        else if (tg.getNumberStubs()/2==p.getNumberElements()) initialiseFromEdgePartition(tg,p);
 ...
 */
     ...
     * <p>If file name ends in <tt>Vertices.dat</tt> ...
    ...
    public void readVertexCommunity(String fullFileName,
        ...
        if (fullFileName.endsWith("Vertices.dat")){
            //int columnVertex=1;
            //int columnCommunity=2;
            //int columnWeight=8; // this should be the fractional S in C column
            String [] columnLabelList = {"Index","Community", "str_C/str"};
            //boolean headerOn=true;
            boolean infoOn=(infoLevel>1);
            String cc="";
            int [] columnReadList= new int[columnLabelList.length];
            ArrayList<ArrayList<String>> vertexData =
    ...
    /**
      ...
      * @param columnWeight  column with community membership fraction
    ...
    public void readVertexCommunityListFile(TreeMap<String,Integer>vertexNameToIndex,
      ...
      FileInput fi = new FileInput(infoLevel);
        ...
        FileInput.readStringEdgeFile(fullFileName,
