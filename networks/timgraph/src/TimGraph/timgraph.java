 ...
 * <code>vertexList[v]</code> is a list of the indexValue of neighbouring vertices stored as a
 ...
 * indexValue of the source vertex of the e-th edge while <code>stubSourceList[2e+1]</code>
 ...
 * the list <code>vertexEdgeList[v]</code> is associating a local indexValue to each stub too.
 ...
 * <br>EdgeValues, which store weights, labels and other edge features,
 * are listed as <code>stubValueList[2e]</code>, each an EdgeValue type for the
 * <tt>(2e)</tt> to <tt>(2e+1)</tt> edge while <code>stubValueList[2e+1]</code> is always
 * <em>exactly the same EdgeValue object</em> even for directed graphs.  Thus updating either
 * <code>stubValueList[2e]</code>  or  <code>stubValueList[2e+1]</code> will work, so don't do this twice!
...
public class timgraph {
     ...
     */
     ...
     /**
      * String used to set default number.
      */
    ...
    /**
     * Current header of vertex indexValue column in input file.
     */
    public String vertexIndexLabel=VertexLabel.VERTEXINDEXLABEL;
    /**
     * Header of vertex indexValue column in input file
     */
    String vertexCommunityLabel="Community";
//    /**
//     * Header of number column in input file
//     */
//    String numberHeader="Number";
//    /**
//     * Header of x position column in input file
//     */
//    String xpositionHeader="Xpos";
//    /**
//     * Header of y position column in input file
//     */
//    String ypositionHeader="Ypos";
...
//    UndirectedGraph g = new UndirectedSparseGraph();
    ...
    private boolean vertexEdgeListOn=false;
     /**
     * List of edges by global edge indexValue connected to a vertex.  
    ...
    IntArrayList[] vertexEdgeList; // 
     ...
     /**
     * List of incoming edges by global edge indexValue incoming to vertex.  
     * <BR>Only defined for directed graphs.
     */
    IntArrayList[] vertexInEdgeList; // 

    /**
     * List of labels for each vertex by global vertex label.
     */
    VertexLabel [] vertexLabelList; // Vertex labels for each vertex
    /**
     * If defined this is a map of vertex name to global indexValue.
     */
    TreeMap<String,Integer> nameToIndex;
    ...
    /**
     * <code>stubValueList[s]</code> is the EdgeValue class entry for stub s.
     * <br>Only defined for weighted graphs.
     * <br>Weight of undirected edge between vertices <code>stubSourceList[2n]</code>
     * and <code>stubSourceList[2n+1]</code> is  <code>stubValueList[2n]</code>
     */
    EdgeValue[] stubValueList ; // values of undirected edge above is  ewl[2n]
     ...
     * <p>Vertices indexValue 0 to <tt>(numberVertexType1-1)</tt> are type one vertices
     */
     ...
     * <p>Vertices indexValue <tt>numberVertexType1</tt> to <tt>(numberVertexType1+numberVerticesType2-1)</tt> 
...
// Parameters for one distance sample fromone source    
    ...
    int componentGCCIndex; // indexValue in arrays of GCC component 
...
//    public final static String [] lgExtensionDescription  = {"Line Graph L(G)", "Line Graph with self-loops", "Weighted Line Graph", "Weighted Line Graph with self-loops"};
     ...
     */
    private int maximumVertices =IUNSET; // 1000;
    /**
     * Absolute maximum number of stubs (twice the maximum number of edges).
     * <p>USed to set array sizes so can not be exceeded.
     */
    private int maximumStubs=IUNSET; //=1000;
...
//    double probrndvertex =0.0; // prob of choosing a random vertex
     ...
     *@param dnameroot root directory number, full path with slash at end e.g. /data/
  ...
  public timgraph(String namert, String dnameroot, int infol, int outputc)
  {
     ...
     * Constructor to set up basic parameters of choice.
     *@param namert root of names used for files e.g. test
     *@param dnameroot root directory number full path with slash at end e.g. /data/ (input and output added to this directory root number)
     *@param infol information level sets information given, >0 for increasing debugging info, <0 for increasing quietness
     *@param outputc controls output levels
          ...
          int maxVertices, int maxStubs)
  {
     ...
     *@param namert root of names used for files e.g. test
     *@param dname directory number full path with slash at end e.g. /data/
     *@param infol information level sets information given, >0 for increasing debugging info, <0 for increasing quietness
     *@param outputc controls output levels
     *@param makeDirected true (false) if want (un)directed graph
            ...
            boolean makeUndirected, boolean makeUnlabelled, boolean reverseDirection) 
    {
        ...
        {   // weighted edges
            for (int e=0; e<oldtg.TotalNumberStubs;) 
            {
                ...
                w1 = oldtg.stubValueList[e++];
                v2 = oldtg.stubSourceList[e];
                w2 = oldtg.stubValueList[e++]; // this should be identical to w1
...
//        outputExtension=oldtg.outputExtension;
     ...
     * <br>The vertex indexValue <tt>v</tt> of the line graph corresponds to the egd with indices
     ...
     * for all edges because and edge alpha is both in coming and outgoing to the vertex v.
...
//        outputExtension=oldtg.outputExtension;
    ...
    /**
     * Network name.
     * Same as start of all input file names (name root).
     * @return name of network
     */
    public String getNetworkName(){ return inputName.getNameRoot();}
    
    /**
     * Sets name root for all input and output files.
     * This is the start of the file name (not including any directories)
     * Uses current input file name root.
     */
      public void setNameRoot()  {
      setNameRoot(inputName.getNameRoot());
  }       
    /**
     * Sets name root for all input and output files.
     * This is the start of the file name (not including any directories)
     * @param root string to be start of all file names.
     */
     ...
     * @param add input and output number roots have this added to their names
     */    
  ...
  /**
   * Root of name with full directory for output files.
   * @return Root of name with full directory for output files.
   */
  public String getNameRootFullPathOutput(){return outputName.getNameRootFullPath();}


    /*
     * Initialises some basic parameters.
     *@param namert root of names used for input files e.g. test
     *@param dnameroot root directory number (full path) to which input or output are appended as appropriate.   (with or without slash at end)
     *@param infol information level sets information given, >0 for increasing debugging info, <0 for increasing quietness
     *@param outputc controls output levels
     */
  public void initialiseSomeParameters(String namert, String dnameroot, int infol, int outputc){
     ...
     *@param indname directory number full path of input files (with or without slash at end)
     *@param outnamert root of names used for output files e.g. test
     *@param outdname directory number full path of output files (with or without slash at end)
     *@param infol information level sets information given, >0 for increasing debugging info, <0 for increasing quietness
     *@param outputc controls output levels
     */
...
// ***********************************************************************    
    ...
    double DoRun(double eventnotefactor)  {
        ...
        int n,nv;
        //int  v,s,k; v=s=k=0; //needed to make catch happy
  ...
  //          System.out.println("Event "+v+", connectivity= "+actualconnectivity);
            ...
            {   
                ...
                if (weightedEdges) 
                {
                    if (newvertexthistime) 
                        stubValueList[TotalNumberStubs]= new EdgeValue(1,destVertAWL[k]);
                    else stubValueList[TotalNumberStubs]= new EdgeValue(2,destVertAWL[k]);
                }
...
//                  vertexLabelList[walkvertex].rank.value2 += diffuseValue;
  ...
  /**
     * Returns the smallest vertex number (a label not an indexValue) if set.
   ...
   /**
     * Returns the largest vertex number (a label not an indexValue) if set.
     * <p>Returns negative value {@value IUNSET} if not set.
     */
  ...
  /**
   ...
   /**
    ...
    public void calcMinimumEdgeValue()
    {
        ...
        for (int s=0;s<TotalNumberStubs;s++) stubValueList[s].setMinimum(minimumEdgeValue);
    }
...

    {
        ...
        for (int s=0;s<TotalNumberStubs;s++) stubValueList[s].setMaximum(maximumEdgeValue);
    }
...
// --------------------------------------------------------------------------
  ...
  public void adjustZeroWalkWeights()
  {
      int zerowalkweight = maxonewalk*2+1;
        for (int e=0; e<TotalNumberStubs; e++)
        {
            if (stubValueList[e].weight==0) stubValueList[e].weight+= zerowalkweight;
        }
      return;
  }
...
// ---------------------------------------------------------      
   /** 
     ...
     * <br>New vertex has indexValue <code>TotalNumberVertices</code>
  ...
  {      
   ...
   /** 
     ...
     * <br>New vertex has indexValue <code>TotalNumberVertices</code> and then this global is incremented.
     */    
...
// ---------------------------------------------------------      
   /** 
     * Adds vertex with vertex label of given of number.
     * <p>Number is set to its indexValue
  ...
  public int addVertex(String name)  {      
   ...
   /** 
     * Adds vertex with vertex label of given of number and number.
     * @param number of vertex
     * @param number of vertex
     */    
...
//      System.out.println("adding edge "+v1+" : "+v2+", TNS "+TotalNumberStubs);
  ...
  public void addEdge(int v1, int v2)  
  {
      ...
      if (weightedEdges) stubValueList[TotalNumberStubs]= new EdgeValue();
      TotalNumberStubs++;
...
//      System.out.println("adding edge "+v1+" : "+v2+", TNS "+TotalNumberStubs);
      ...
      addEdgeSourceBasic(v1, v2);
      stubValueList[TotalNumberStubs++]= edgeweight;
...
//      System.out.println("adding edge "+v1+" : "+v2+", TNS "+TotalNumberStubs);
      addEdgeSourceBasic(v1, v2);  
      stubValueList[TotalNumberStubs++]= edgeweight1;
      
      addEdgeTargetBasic(v1, v2);
      stubValueList[TotalNumberStubs++]= edgeweight2;
  }       
    /**
    ...
    public void increaseEdgeWeight(int s, int t, double dw) {
        ...
        if (infoLevel > 2) {
            ...
            if (weightedEdges) {
                ...
                System.out.println(", edge index "+(e-1)+", has had weight increased by " + dw + " to become " + stubValueList[e-1].weight);
                System.out.print("Edge from " + s + " to " + t);
                System.out.println(", edge index "+(e)+", has had weight increased by " + dw + " to become " + stubValueList[e].weight);
            } else {
...
//
    ...
    /**
     * Creates a new edge but only if such an edge does not already exist.
     * <br>Uses local vertex edge indexValue so works even is <code>vertexEdgeListOn</code> is false.
     * @param s source vertex
     * @param t target vertex
...
// ---------------------------------------------------------      
  ...
  {    
   ...
   int e;
   stubSourceList = new int[TNS];
   if (weightedEdges) stubValueList = new EdgeValue[TNS];
   TotalNumberStubs=0;
   int n;
   ...
   { 
       ...
       if (oldtg.stubValueList[e].label != keeplabel ) continue;
       stubValueList[TotalNumberStubs]= new EdgeValue(oldtg.stubValueList[e]);
       stubSourceList[TotalNumberStubs++] =oldtg.stubSourceList[e++];
       stubValueList[TotalNumberStubs]= new EdgeValue(oldtg.stubValueList[e]);
       stubSourceList[TotalNumberStubs++] =oldtg.stubSourceList[e];
...
// Adapted to sort indexValue array
    ...
    * the lowest indexValue on the left to the highest indexValue on the right
    * then the parameters to this function are lowest indexValue or
    * left and highest indexValue or right.  The first time you call
...
// ---------------------------------------------------------      
  ...
  public void calcRandomEdgeWeightOrder(int criteria)  
  {
    ...
    *@param a returned as list of indices of stubValueList in ranked order.
    *@param lo0 the starting indexValue
    *@param hi0 the maximum indexValue (inclusive)
    */      
    private void QuickSort(int criteria, int a[], int lo0, int hi0)
   {
      ...
      if ( hi0 > lo0)
      {
          ...
          * the array.
          */
          mid = stubValueList[a[( lo0 + hi0 ) / 2]*2];
//         mid = a[ ( lo0 + hi0 ) / 2 ];
//         midindex = ( lo0 + hi0 ) / 2 ;
         ...
         while( lo <= hi )
         {
            ...
            while( ( lo < hi0 ) && ( compareWeights(criteria, stubValueList[a[lo]*2] , mid ) ))
               ++lo;
            ...
            while( ( hi > lo0 ) && ( compareWeights(criteria, mid, stubValueList[a[hi]*2]  ) ))
               --hi;
         ...
         /* If the right indexValue has not reached the left side of array
          * must now sort the left partition.
          */
         if( lo0 < hi )
            QuickSort(criteria, a, lo0, hi );

         /* If the left indexValue has not reached the right side of array
...
// -----------------------------------------------------------------------       
  ...
  //* @param componentVertexList is a complete list of all vertices visited  v.e.
  ...
  /**
     ...
     * v.e. those v at <tt>(distance+1)</tt>, and marks them as -1 in vertexdistance[v]
...
// check found all vertices and find average size
    ...
    /**
     * Makes a TreeSet of vertices in a ring, v.e. distance <tt>maxDistance</tt> from <tt>vertex</tt>.
     * <p>Useful for the subgraph projection.
    ...
    public TreeSet<Integer> getEdgeSubGraph(Set<Integer> labelSet){
        ...
        for (int e=0; e<TotalNumberStubs; e+=2) 
            if (labelSet.contains(stubValueList[e].label) ) {
...
// *********************************************************
     ...
     * Sets distance[v][j] to shortest distance from v to j
    ...
    public void calcStrength(){
        ...
        if (this.vertexEdgeListOn){
            ...
            for (int v=0; v<TotalNumberVertices; v++) {
                ...
                sin=this.getVertexInStrength(v);
                vertexLabelList[v].setStrength(s);
                vertexLabelList[v].setInStrength(sin);
                TotalWeight+=s;
            }
            return;
        }
        ...
        for (int e=0; e<TotalNumberStubs; e++) {
            ...
            if (!isDirected()) { // target vertex
             ...
             if (s!=t) {
                ...
                vertexLabelList[s].addInStrength(w);
                vertexLabelList[s].updateMaxInWeight(w);
            }
        }    
...
//        double rankingProbability=(1.0 / (1.0+(1.0/rankingProbabilityLengthScale) ) );
    ...
    public void calcStructuralHoleVertexData(){
        setVertexLabelsOn();
        if (!this.vertexEdgeListOn) {throw new RuntimeException("*** Error calcStructuralHoleVertexData requires vertexEdgeList on");}
...
// -----------------------------------------------------------------------       
    /**
     * Adds value to the element at indexValue.
     * Adds value to the element at indexValue extending with zero entries
     * or starting list if necessary
     * @param ial IntArrayList
     * @param indexValue the indexValue in the IntArrayList
     * @param value value to set:  ial[indexValue]=value
     */
...
// **************************************************************************
    ...
    /**
     * Tests to see if vertex indexValue is in graph.
     * @param v global vertex indexValue to test
     * @return true (false) if vertex indexValue is (not) allowed
     */
    ...
    /**
     * Returns true (false) if infoLevel is (not) greater than specified integer v.
     * @param v the integer
     * @return true (false) if infoLevel is (not) greater than  v.
    ...
    /**
     * Sets number and type number of bipartite vertices.
     * @param n1 number of type 1 vertices
     * @param n2 number of type 2 vertices
     * @param name1 number of type 1 vertices
     * @param name2 number of type 2 vertices
     */
     ...
     * @param root input and output number roots are set to be this.
     */    
...
// ---------------------------------------------------------      
  ...
  public int getNumberVerticesType2()  {
   ...
   /**
    * Returns number of vertices of type 1 in a bipartite timgraph.
    *  @return number of vertices of type 1.
     */
  public String getNameVerticesType1()  {
   ...
   /**
    * Returns number of vertices of type 2 in a bipartite timgraph.
    *  @return number of vertices of type 2.
     */
...
// ---------------------------------------------------------      
    ...
    * Note this is the indexValue e for <code>stubSourceList[e]</code>.
    * It is NOT the local indexValue e for <code>vertexList[s].get(e)</code>.
    * <br><code>vertexEdgeList</code> must be on.
    * @param s source vertex
    * @param t target vertex
    * @return global edge indexValue of first edge where <code>vertexList[s]=t</code>, -1 if none found.
     */
  ...
  public double getAdjacencyMatrixEntry(int s, int t)  {
   ...
   /** 
     * Finds global edge indexValue from given vertex and its local edge indexValue.
    * <br><code>vertexEdgeList</code> must be on but this is not checked.
    * @param s vertex
    * @param elocal local edge label of vertex s
    * @return global indexValue of edge
...
// ---------------------------------------------------------      
   /** 
    * Finds edge from given vertices.
    * <br>Note this is the indexValue e in <code>vertexList[s].get(e)</code>.  
    * It is NOT the global indexValue e for <code>stubSourceList[e]</code>.
    * @param s source vertex
    * @param t target vertex
    * @return local edge indexValue of first edge where <code>vertexList[s]=t</code>, -1 if none found.
     */    
...
// ---------------------------------------------------------      
    ...
    * <br>Note this is the indexValue e in <code>vertexList[s].get(e)</code>.  
    * It is NOT the global indexValue e for <code>stubSourceList[e]</code>.
    * @param einitial initial indexValue of edge in  <code>vertexList[s]</code>
    * @param s source vertex
    * @param t target vertex
    * @return local edge indexValue of first edge where <code>vertexList[s]=t</code>, -1 if none found, -2 if initial edge out of bounds.
     */    
  ...
  /**
   * Given an indexValue in the local list of edges attached to a vertex (local stub indexValue), returns the global edge indexValue.
   * <br>If directed this is an outgoing edge.
   * <br>No checks done.
   * @param v the global vertex indexValue.
   * @param localEdgeIndex the local indexValue of a stub in the <tt>vertexEdgeList[v]</tt>
   * @return The global edge indexValue <tt>vertexEdgeList[v].getQuick(localEdgeIndex)</tt>
  ...
  /**
   * Given an indexValue in the local list of incoming edges attached to a vertex (local stub indexValue), returns the global outgoing stub indexValue.
...
// ---------------------------------------------------------      
   /** 
     * Returns the stub at the other end of edge e, v.e. flips last bit of e.
     *@param e edge number
  ...
  /**
   * Returns stub indexValue of edge associated with given stub
   * @param s stub indexValue
...
// ---------------------------------------------------------      
   /** 
     ...
     *@return  the global indexValue of the vertex at end of stub s
     */    
...
// ---------------------------------------------------------      
   /** 
     ...
     * <br>Ignores directionality
     *@param s global stub indexValue
     *@return the global indexValue of the vertex at other end of edge specified by the stub
     */    
...
// ---------------------------------------------------------      
  ...
  /**
   * Finds global vertex indexValue from number of vertex.
   * <p>Looks for a vertex label starting with given number, case is ignored.
   * Note vertex labels must be present.
   * TODO use a vertex number to vertex indexValue map
   * @param vertexName look for first vertex whose label starts with this string, ignoring case.
   * @return vertex indexValue.  If none found, then this will equal the total number of vertices.
   */
  ...
  /**
   * Set TreeMap from vertex name to global vertex index.
   * @return TreeMap from vertex name to global vertex index, null if no labels.
   */
  public TreeMap<String,Integer>  setVertexNameToIndexMap(){
      //if (!vertexlabels) throw new IllegalArgumentException("graph with labelled vertices needed to find named vertex.");
      if (!vertexlabels) {nameToIndex=null; return null; }
  ...
  /**
   * Get TreeMap from vertex name to global vertex index.
   * @return TreeMap from vertex name to global vertex index.
   */
  public TreeMap<String,Integer> getVertexNameToIndexMap(){
      return nameToIndex;
  }
...
// ---------------------------------------------------------
    ...
    * <p>If vertices have no labels then their number is given as the number.
    * @param v vertex number
    * @return string with some sort of number for the vertex.
    */
  ...
  /**
    ...
    * <p>If vertices have no labels then their number is given as the number.
    * @return array of strings with some sort of number for each vertex.
    */
...
// ---------------------------------------------------------
    ...
    * a label (e.g. community label) and not the indexValue of the vertex.
...
// ---------------------------------------------------------
  ...
  public double getVertexMaxWeight(int v)  {
      ...
      {
          edge= vertexEdgeList[v].get(e);
          maxw=Math.max(maxw, stubValueList[edge].weight);
      }
      return maxw;
  }
...
// ---------------------------------------------------------
  ...
  public double getVertexSelfLoopStrength(int v)  {
      ...
      {
          edge= vertexEdgeList[v].get(e);
          if (isStubSelfLoop(edge)) str+= (weightedEdges ?stubValueList[edge].weight:1.0);
      }
      return str;
  }
...
// ---------------------------------------------------------      
   /**
    * Returns in strength of a vertex of a directed graph.
  ...
  public double getVertexInStrength(int v)  {
      if (!directedGraph) {
          return ( getVertexOutStrength(v));
      }
      if (!weightedEdges) {
          return ( getVertexInDegree(v));
      }
      int edge; 
      double s=0;
  ...
  public  int [] getStubSourceList()  {
    ...
    * <br>No tests done on input or graph type.
     ...
     */
  public double getEdgeWeightQuick(int s)  {
      return (stubValueList[s].weight);
  }
   /** 
    * Returns weight of an stub in a weighted timgraph.
    * <br>Returns 1.0 if graph has no edge weights
    * <br>No tests done on input.
     * @param s stub number
     * @return weight of edge
     */    
  public double getEdgeWeight(int s)  {
      return (weightedEdges?stubValueList[s].weight:1.0);
  }       

   /** 
    * Returns weight of an edge in a weighted timgraph.
    * <br>Returns 1.0 if graph has no edge weights
    * <br>If edge indexValue is out of range returns <tt>timgraph.DUNSET</tt>
     * @param s global stub indexValue
     * @return weight of edge
     */    
  public double getEdgeWeightSlow(int s)  {
      if ((s<0) || (s>=TotalNumberStubs)) return DUNSET;
      return (weightedEdges?stubValueList[s].weight:1.0);
  }       
  ...
  public EdgeValue getEdgeWeightAll(int e)  {
      return (stubValueList[e]);
  }       

  
  // ---------------------------------------------------------      
   /**
    * Returns global indexValue of neighbouring vertex.
    * <br>Checks that vertex is in range. Works for all graphs.
    * If directed graph this only follows edges in direction of edge
    * v.e. this gets vertices which are thee targets of edges with
    * source input vertex v.
    * @param v global indexValue of vertex for which neighbours are required
    * @param e local indexValue of neighbours
    * @return indexValue of e-th neighbouring vertex to v or -1 if no neighbours
     */
  ...
  public int getVertexTargetQuick(int v, int e)  {
    ...
    /**
     * Returns global indexValue of neighbouring source vertex.
  ...
  /**
    ...
    * @param v vertex for which neighbours are required
    * @param e local indexValue of sources neighbours
    * @return indexValue of e-th neighbouring vertex to v or -1 if no neighbours
     */
 ...
 // ---------------------------------------------------------      
   ...
   * @param v global vertex indexValue
   ...
   * <br>The vertex indexValue <tt>v</tt> of the given community (and of the line graph)
  ...
  public void setEdgeLabels(Partition c)  {
      ...
      if (!weightedEdges){
          weightedEdges=true;
          stubValueList = new EdgeValue[maximumStubs];
          for (int e=0; e<TotalNumberStubs; e++) {
      ...
      for (int v=0; v<cov.length; v++) {
          stubValueList[e++].setLabel(cov[v]);
          stubValueList[e++].setLabel(cov[v]); // except for e++, should be redundant as access the same object.
      }
  }       
  /**
  ...
  public void setEdgeLabels(int value)  {
      ...
      labelledEdges=true;
      for (int e=0; e<TotalNumberStubs; e++) {
          ew = stubValueList[e++]; // this sets e and e+1 as they are same object
  ...
  public void setEdgeWeights(int value)  {
   ...
   /**
     * Sets all the edge weight of given stub.
     * @param gs global stub label (no checks done)
     * @param value value for edge weight
     */
  public void setEdgeWeight(int gs, double value)  {
          stubValueList[gs].weight=value;
      return;
  }
   ...
   * Sets positions for all vertices.
   * <br>Initialises new <tt>vertexLabelList</tt> if does not exist.
   * @param coord list of x then y coordinates as strings
   */
  ...
  /**
   * Sets numbers for all vertices.
   * <br>Initialises new <tt>vertexLabelList</tt> if does not exist.
   * If indexValue is outside allowed range then warns, ignores but continues.
   * If vertex indexValue is allowed but does not exist it is created.
   * If vertex exists then it is given new coordinate overriding any existing one.
   * Illegal coordinates are flagged, not used but processing continues.
   * If indexValue list is empty or null, the vertices indexed in order they are given
   * starting from 0.
   * @param indexList list of vertex indices
   * @param xl list of strings containing x coordinates
   * @param yl list of strings containing y coordinates
   */
  public void setVertexPositionsFromStrings(int [] indexList,
          ArrayList<String> xl, ArrayList<String> yl)  {
      if ( xl.isEmpty() || yl.isEmpty()  ) return;
      boolean useIndexList=true;
      if ((indexList==null) || (indexList.length==0)) useIndexList=false;
      else if ((xl.size()!=indexList.length)&&(yl.size()!=indexList.length)  )
            throw new RuntimeException ("*** Number of x or y coordinates differ from number indices, x: "
                    +xl.size()+", y:"+xl.size()+", indices:"+indexList.length);
      if (!this.vertexlabels) initialiseVertexLabels();
      int v=IUNSET;  //global indexValue to vertex to be updated
      Double x=DUNSET;
      Double y=DUNSET;
      Warning warning = new Warning();
      int nSet=0;
      for (int i=0; i<Math.max(xl.size(),yl.size()); i++) {
          if (useIndexList) v=indexList[i];
          else v=i;
          if (v>=this.maximumVertices || v<0) {
              warning.nextErr("vertex index "+v+
                      " outside allowed range of 0 to "+maximumVertices+", ignored");
              continue;
          }
          while (v>=this.TotalNumberVertices) this.addVertex(); // add missing vertices
          try{
              x=Double.parseDouble(xl.get(v));
              y=Double.parseDouble(yl.get(v));
              vertexLabelList[v].setPosition(x, y);
              nSet++;
          }
          catch(Exception e){
              warning.nextErr("setVertexPositionsFromStrings found vertex "
                      +v+", x,y= "+x+","+y+" so ignored");
          }
      } // eo for v
      System.out.println("Set "+nSet+" positions");
  }
   ...
   * @param yl list of strings containing y coordinates
   */
  public void setVertexPositionsFromStrings(ArrayList<String> xl, ArrayList<String> yl)  {
      if ((xl.size()!=2*TotalNumberVertices)  )
            throw new RuntimeException ("Wrong number of x coordinates, found "+xl.size());
      if ((yl.size()!=2*TotalNumberVertices)  )
            throw new RuntimeException ("Wrong number of y coordinates, found "+yl.size());
      if (!this.vertexlabels) initialiseVertexLabels();
      int c=0;
      double x=DUNSET;
      double y=DUNSET;
      ...
      for (int v=0; v<this.TotalNumberVertices; v++) {
         ...
         * @param fullFileName full file number of file with positions of vertices.
    ...
    /**
     * Set name of vertex
     * <p>No checks done, needs labels to be on
     * @param v globalindex of vertex
     * @param newName new name for vertex
     * 
     */
    public void setVertexName(int v, String newName){
        this.vertexLabelList[v].setName(newName);
    }
  /**
   * Sets names for vertices.
   * <br>vertex names are in indexValue order.
   * Initialises new <tt>vertexLabelList</tt> if does not exist.
   * Will always apply names even if list given not equal to number of
   * vertices. If too few existing vertices, more vertices added and named.
   * If to many existing vertices, names used until run out and rest names
   * left unchanged.
   * @param nameList list of names as strings in indexValue order
   * @see #setVertexNames(int[], java.util.ArrayList)
   * @deprecated
   */
  public void setVertexNamesOld(ArrayList<String> nameList)  {
      if ((nameList.size()!=TotalNumberVertices)  )
            System.err.println("!!! WArning given "+nameList.size()+" names but have "+getNumberVertices()+"vertices");
      if (!this.vertexlabels) initialiseVertexLabels();
      int c=0;
      String name="UNSET";
      for (int v=0; v<nameList.size(); v++) {
          if ((v==this.getNumberVertices()) && (v<this.getMaximumVertices())) addVertex();
   ...
   * @see #setVertexNames(int[], java.util.ArrayList)
   * @param indexList list of vertex indices
   * @param nameList list of names in indexList order
   */
  public void setVertexNames(ArrayList<String> nameList)  {
      setVertexNames(null, nameList);
  }
  /**
   ...
   * If indexValue is outside allowed range then warns, ignores but continues.
   * If vertex indexValue is allowed but does not exist it is created.
   * if vertex exists but does not have a number then given number is used.
   * If vertex number exists, an error is given is new and given names not identical.
   * If indexValue list is empty or null, the vertices indexed in order they are given
   * starting from 0.
   * @param indexList list of vertex indices
   * @param nameList list of names in indexList order
   */
  public void setVertexNames(int [] indexList, ArrayList<String> nameList)  {
      if ((nameList.size()!=indexList.length)  )
            throw new RuntimeException ("*** Number of names and indices differ "+nameList.size()+", "+indexList.length);
      if (!this.vertexlabels) initialiseVertexLabels();
      int v=IUNSET; // global indexValue of vertex to be updated
      String name="UNSET";
      Warning warning = new Warning();
      int nSet=0;
      int nUnchanged=0;
      ...
      for (int i=0; i<nameList.size(); i++) {
          ...
          if (v>=this.maximumVertices || v<0) {
              warning.nextErr(i+"th vertex index "+v+
                      " outside allowed range of 0 to "+maximumVertices+", ignored");
              continue;
          }
          while (v>=this.TotalNumberVertices) this.addVertex(); // add missing vertices
          try{
              name=nameList.get(i);
              // if number exists check identical.  Otherwise add new number.
              if (this.vertexLabelList[v].hasName()) {
                  if (vertexLabelList[v].getName().equals(name)) {nUnchanged++;}
                  else {
                      warning.nextErr(i+"th vertex, index "+v
                          +", has existing name "+vertexLabelList[v].getName()
                          +", does not match new name "+name);
                  }
              } else {
                 vertexLabelList[v].setName(name);
                 nSet++;
              ...
              warning.nextErr("setVertexNames, vertex index "+v+" name "+name+" failed, "+e);
              name="UNSET";
          }
      } // eo for v
            System.out.println("Set "+nSet+" new names and found "+nUnchanged+" mathcing existing names");
  }

        /**
         * Reads in vertex properties from file.
         * <p>Standard extension is <tt>inputVertices.dat</tt>.
         * <p>Each line has entries separated by whitespace.
         * Any lines starting with the comment string (after any white space) are skipped.
         * Columns probably numbered with first being number 1.
         * <p>Vertices need not already exist though typically edges are set up first.
         * If indices are given these are used to assign given data to vertices.
         * If names are given, these are matched exactly to existing names.
         * If neither names or indices given then it is assumed vertices appear
         * in the indexValue order starting from 0.  Generally if vertex doesn't
         * exist it is created.  If it is outside maximum range then warning given but
         * processing continues.
         * @param ext extension to add to find file with names, inputVertex.dat for standalone files.
         * @param cc comment character, if first no white space characters are this string, then line is ignored.
         * @param nameColumn position of names from this column
         * @param headerOn ignore first line if true
         * @param infoOn true if want the input lines printed out as read in
         */
    public void setVertexPropertiesFromFile(String ext, String cc,  
            boolean headerOn, boolean infoOn)
    {
        String fullFileName=inputName.getNameRootFullPath()+ext;
        //int [] columnReadList = null;
        boolean forceLowerCase=false;
        String [] labelList = {this.vertexIndexLabel, VertexLabel.NAMELABEL,VertexLabel.NUMBERLABEL,
                               Coordinate.XLabel, Coordinate.YLabel, Coordinate.ZLabel};
        final int indexIndex=0;
        final int nameIndex=1;
        final int numberIndex=2;
        final int xIndex=3;
        final int yIndex=4;
        final int zIndex=5;
        ArrayList<ArrayList<String>> propertyList = FileInput.readStringColumnsFromFile(fullFileName,
            cc, null, labelList, forceLowerCase, infoOn);

        int [] indexList;
        int nv=-1;
        Warning warning = new Warning();
        int maxIndex=0;
        if ((propertyList.get(indexIndex)!=null) && (!propertyList.get(indexIndex).isEmpty())) {
            // vertex indexValue given so use to map onto properties
            indexList = new int[propertyList.get(indexIndex).size()];
            int r=0; // data row in file
            int indexValue=IUNSET;
            for (String s: propertyList.get(indexIndex)) {
                try{
                    indexValue=Integer.parseInt(s);
                    } catch (RuntimeException e){
                    indexValue=IUNSET;
                    warning.nextErr("Data row "+r+" index "+s+" not integer");
                    }   
                indexList[r++]=indexValue;
                maxIndex=Math.max(indexValue, maxIndex);
            }// eo for s
        } // eo if (propertyList.get(indexIndex).size()>0)
        else{ // if no indexValue use given names
            if ((propertyList.get(nameIndex)!=null) && (!propertyList.get(nameIndex).isEmpty())){
                // no indexValue given so use given names
                if (nameToIndex==null) {
                    setVertexNameToIndexMap();
                }
                indexList = new int[propertyList.get(nameIndex).size()];
                int r=0; // data row in file
                int indexValue=IUNSET;
                int newVertexNumber=getNumberVertices();
                for (String s: propertyList.get(nameIndex)) {
                    try{
                       indexValue=nameToIndex.get(s);
                    } catch (RuntimeException e){
                       indexValue=newVertexNumber++; // assign next available new vertex number
                       warning.nextOut("Data row "+r+" name "+s+" unknown");
                    }
                    indexList[r++]=indexValue;
                    maxIndex=Math.max(indexValue, maxIndex);
                }// eo for s
            } // if (propertyList.get(nameIndex).size()>0)
            else{ // no indexValue and no name, so treat as being in numerical order
                indexList=null;
            }
        }
        //setMaximumVertices(Math.max(maximumVertices,maxIndex));
        //setNetwork();
        setVertexNames(indexList,propertyList.get(nameIndex));
        setVertexNumbers(indexList,propertyList.get(numberIndex));
        setVertexPositionsFromStrings(indexList,propertyList.get(xIndex),propertyList.get(yIndex));
    }

    /**
     * Sets up vertex properties from many input files.
     * Looks for <tt>inputXY.dat</tt> to set x and y positions of vertices and
     ...
     * @return
     */
    public int setVertexPropertiesFromManyFiles(int xColumn, int yColumn, int nameColumn, boolean headerOn, boolean infoOn) {
     ...
     * @param headerOn
     * @param infoOn
     * @depracted use {@link #setVertexPropertiesFromFile(java.lang.String, java.lang.String, boolean, boolean) }
     */
    public void setVertexPropertiesFromFileOLD(String ext, String cc,
            int numberColumn, boolean headerOn, boolean infoOn)
    {
        String fullFileName=inputName.getNameRootFullPath()+ext;
    ...
    public void setVertexNamesFromFile(String ext, String cc, int nameColumn, boolean headerOn, boolean infoOn)
    {
         ...
         * the same as its indexValue.  For instance used for ranking vertices or forming partitions.
   ...
   * If vertex indexValue is allowed but does not exist it is created.
   * if vertex exists but does not have a number then given number is used.
   * If vertex number exists it is overwritten without warning.
   * If indexValue list is empty or null, the vertices indexed in order they are given
   * starting from 0.
   * @param indexList list of vertex indices
   * @param numberList list of numbers in indexList order
   */
  public void setVertexNumbers(int [] indexList, ArrayList<String> numberList)  {
      if (numberList==null || numberList.isEmpty()  ) return;
      boolean useIndexList=true;
      if ((indexList==null) || (indexList.length==0)) useIndexList=false;
      else if ((numberList.size()!=indexList.length)  )
            throw new RuntimeException ("*** Number of numbers and indices differ "+numberList.size()+", "+indexList.length);
      if (!this.vertexlabels) initialiseVertexLabels();
      int v=IUNSET; // global indexValue of vertex to be updated
      Integer number;
      ...
      for (int i=0; i<numberList.size(); i++) {
          ...
          while (v>=this.TotalNumberVertices) this.addVertex(); // add missing vertices
          try{
              number=Integer.parseInt(numberList.get(i));
                 vertexLabelList[v].setNumber(number);
                 nSet++;
//              // if number exists check identical.  Otherwise add new number.
//              if (this.vertexLabelList[v].hasNumber()) {
//                  if (vertexLabelList[v].getNumber()!=number) throw new
//                          RuntimeException ("*** Numbers for vertex "+v
//                          +" do not match, existing: "+vertexLabelList[v].getNumber()
//                          +", new: "+number);
//              } else {
//                 vertexLabelList[v].setNumber(number);
//              }
              ...
              warning.nextErr("setVertexNumbers "+i+"th vertex, index "+v+", number "+numberList.get(v)+", ignored, "+e);
          }
      } // eo for v
            System.out.println("Set "+nSet+" numbers");
...
//     * Note that as always <tt>stubValueList[2e]</tt> and
//     * <tt>stubValueList[2e+1]</tt> are the same single object
...
// FILE INPUT ROUTINES
     ...
     *@param fileName full number inclduing directory and extension for input file
            ...
            for (int e=0; e<TotalNumberStubs; e+=2)  
                {
                    ...
                    if (weightedEdges) PS.println("  " 
                               + stubValueList[e].weight
                               + "  c " + getPajekColour(stubValueList[e].label) );
                    else PS.println();
                }
...
//        System.out.println(args.length+" command line arguments");
     ...
     * and takes the file number ending to be <tt>param.dat</tt>
     */
                            ...
                            switch (argument.charAt(1)) {
                                ...
                                case 'f': {
                                        ...
                                        { // input and output are set
                                            ...
                                            outputName.setFullFileName(argument.substring(4),InputFileType.extensionList);
                                        }
                                        if (argument.charAt(3)=='d' ) {
                                            inputName.setDirectoryRoot(argument.substring(4));
                                            outputName.setDirectoryRoot(argument.substring(4));}
                                        if (argument.charAt(3)=='s' ) {
                                            inputName.setDirectoryEnd(argument.substring(4));
                                            outputName.setDirectoryEnd(argument.substring(4));}
                                        if (argument.charAt(3)=='r' ) {
                                            inputName.setNameRoot(argument.substring(4));
                                            outputName.setDirectoryEnd(argument.substring(4));}
                                        if (argument.charAt(3)=='e' ) {
                                            inputName.setNameEnd(argument.substring(4));
                                            outputName.setDirectoryEnd(argument.substring(4));}                                        
                                    }
                                ...
                                case 'g': { 
                                    if (argument.charAt(2)=='v' ){
                                        if (argument.charAt(3)=='b' ) bipartiteGraph = (argument.charAt(4) =='t'?true:false);
                                        if (argument.charAt(3)=='1' ) nameVertexType1 = (argument.substring(4));
                                        if (argument.charAt(3)=='2' ) nameVertexType2 = (argument.substring(4));
                                    ...
                                    if (argument.charAt(2)=='e' ){
                                        if (argument.charAt(3)=='d' ) directedGraph = (argument.charAt(4) =='t'?true:false);
...
//                                case 'v': {infoLevel = Integer.parseInt(argument.substring(2));  break;}
...
// -------------------------------------------------------------------
 ...
 /**
   ...
   */
   public static void printUsage()  {printUsage(System.out);}
   ...
   public static void printUsage(PrintStream PS)  {
        ...
        RandomWalk rw = new RandomWalk(temp);
                ...
                PS.println(" -f?n<FullName> full filename, directories optional, ?=a all, ?=i input or ?=o output, default input="+temp.inputName.getNameRoot()+", output="+temp.outputName.getNameRoot());
                PS.println(" -f?r<NameRoot> root of filename, ?=a all, ?=i input, or ?=o output, default input="+temp.inputName.getNameRoot()+", output="+temp.outputName.getNameRoot());
                PS.println(" -f?e<Ending>  ending used for files, ?=a all, ?=i input or ?=o output, default  input="+temp.inputName.getNameEnd()+", output="+temp.outputName.getNameEnd());
                PS.println(" -f?s<SubDirName>  sub directory using forward slashes for ?=a all, ?=i input or ?=o output, default  input="+temp.inputName.getDirectoryEnd()+", output="+temp.outputName.getDirectoryEnd());
                PS.println(" -f?d<DirName>  root of directory using forward slashes for ?=a all, ?=i input or ?=o output, default  input="+temp.inputName.getDirectoryRoot()+", output="+temp.outputName.getDirectoryRoot());
                ...
                PS.println(" -gvb<String> make vertices bipartite, \"t\"=true, otherwise false, default "+(temp.isDirected()?true:false) );
                PS.println(" -gv1<String> name of type one vertices in a bipartite graph, default "+temp.getNameVerticesType1() );
                PS.println(" -gv2<String> name of type two vertices in a bipartite graph, default "+temp.getNameVerticesType2() );
                ...
                PS.println(" -ged<String> make edges directed, \"t\"=true, otherwise false, default "+(temp.isDirected()?true:false) );
                PS.println(" -gel<String> make edges labelled, \"t\"=true, otherwise false, default "+(temp.isEdgeLabelled()?true:false) );
...
// ...........................................................................   
     ...
     */
     void FileOutputNetwork(boolean printTriangles, boolean printSquares, boolean printNearestNeighbours)
     {
        ...
        try {
            ...
            printEdges(PS);            
            if (infoLevel>-2) {
                System.out.println("Finished writing Network list to "+ outputName.getFullFileName());
            }
            try{ fout.close ();   
               } catch (IOException e) { System.out.println("File Error");}
     ...
     * @param printTriangles prints number of triangles
     * @param printSquares print number of squares but only if printTriangles also true
     * @see FileOutput#printVertices(java.lang.String, java.lang.String, boolean, boolean, TimGraph.Community.VertexPartition, boolean)
     * @deprecated
     */
     void FileOutputVertices(boolean printTriangles, boolean printSquares, boolean printNearestNeighbours)
     {
        ...
        try {
            ...
            printVertices(PS, printTriangles, printSquares, printNearestNeighbours);
            if (infoLevel>-2) {
                System.err.println("Finished writing vertex information to "+ outputName.getFullFileName());
            }
            try{ fout.close ();   
...
// ...........................................................................     
     ...
     * @param printSquares print number of squares but only if printTriangles also true
     * @param printName true (false) to (not) print  number
     * @param printNumber true (false) to (not) print number
     * @param printPosition true (false) to (not) print coordinate
     * @param printType true (false) to (not) type number if bipartite (ignored if not bipartite)
     ...
     */
             ...
             boolean printNearestNeighbours)
     {
         ...
         //PS.println(sep+"No. Vertices = "+sep+TotalNumberVertices);
         for (int v=0; v<TotalNumberVertices; v++) {
             PS.println(getVertexString(cc, sep, v,
                        (printCommunityLabel?vp.getCommunity(v):IUNSET), 
                        printTriangles, printSquares,
    ...
    /**
     * Prints vertex information on PrintStream for several vertex partitions.
     * <p>Will try to print out items even if they are not defined.  Use
     * <tt>printName=hasName()</tt> to avoid this or other <tt>printVertices</tt> routines.
     * @param PS PrintStream such as System.out
     * @param cc comment string
     * @param sep separation string
     * @param vpLabel single string for header of all vertex partition columns
     * @param vpList list of vertex partitions
     * @param vp vertex partition, null if not to be shown
     * @param printTriangles print number of triangles
     * @param printSquares print number of squares but only if printTriangles also true
     * @param printName true (false) to (not) print  number
     ...
     */
     public void printVertices(PrintStream PS, String cc, String sep,
             String  vpLabel, ArrayList<VertexPartition> vpList,
             boolean printTriangles, boolean printSquares,
             boolean printName, boolean printNumber, boolean printPosition,
             boolean printType,
             boolean printStrength, boolean printMaxWeight,
             ...
             boolean printNearestNeighbours)
     {
         boolean printCommunityLabel = false; //(vp !=null);
         PS.print(getVertexStringLabel(cc, sep,
              printTriangles, printSquares,
              printCommunityLabel, printName, printNumber, printPosition,
              printType,
              ...
              printClusterCoef,
              printRank, printStructuralHoleData, printNearestNeighbours));
         PS.println(sep+vpLabel);
         for (int v=0; v<TotalNumberVertices; v++) {
             PS.print(getVertexString(cc, sep, v,
                    IUNSET, printTriangles, printSquares,
                    printName, printNumber, printPosition,
                    printType,
                    printStrength, printMaxWeight,
                    printClusterCoef,
                    printRank, printStructuralHoleData,
                    printNearestNeighbours));
             for (VertexPartition vp: vpList){
                 try{PS.print(sep+vp.getCommunity(v));}
                 catch(RuntimeException e){PS.print(sep+IUNSET);}
             }
             PS.println();
         }
     }
     ...
     */
             ...
             boolean printNearestNeighbours)
     {
         String s=Integer.toString(v);
            ...
            if (this.bipartiteGraph && printType) s=s+sep+(isType1(v)?1:2);
            if (vertexlabels) s=s+sep+vertexLabelList[v].printString(sep, 
                    printName, printNumber, printPosition,
                    printStrength,(printStrength & isDirected()), printMaxWeight,
                    printClusterCoef,
                    printStructuralHoleInfo, printRank);
            else if (printStrength) {
                s=s+sep+this.getVertexOutStrength(v);
                if (isDirected()) s=s+sep+this.getVertexInStrength(v);
            }
...
//            }
     ...
     */
     public String getVertexStringLabel(String cc, String sep, 
             ...
             boolean printNearestNeighbours)
     {
            String s=this.vertexIndexLabel;
            if (printCommunityLabel) s=s+sep+this.vertexCommunityLabel;
            s=s+sep+"Degree";
            if (printTriangles) {
                s=s+sep+"Triangles";
                if (printSquares) s=s+sep+"Squares";
            }
            if (this.bipartiteGraph && printType) s=s+sep+"Type";
...
// ...........................................................................
     ...
     * Prints a list of the edges to printstream.
      * <br>Each line has one edge with the edge indexValue of source, edge indexValue of target, 
      * vertex indexValue of source, vertex indexValue of target, edge label, edge weight.
      * Target edge indexValue should be one more than its source and the source indices ought to be even. 
      * @param PS PrintStream such as System.out
      ...
      * @param edgeIndexOn if true the the first two items will be the edge indexValue used internally.  
             ...
             boolean vertexNames, boolean infoOn, boolean headerOn, boolean edgeIndexOn, boolean edgeLabelOn) 
     {
            ...
            {
                nextline="";
                if (weightedEdges ) {ew=stubValueList[e].weight;
                                     if ((testWeight) && (ew<minWeight)) continue;
                }
                if (weightedEdges && edgeLabelOn) {el=stubValueList[e].label;
    ...
    * <tt>-v-1</tt> where <tt>v</tt> is the indexValue of the vertex.
...
//            }
        ...
        for (int ei=0; ei<edgeList.size(); ei++){
            ...
            int c= this.stubValueList[e].label;
            double w= this.stubValueList[e].weight;
            s+=w;
    ...
    public void printOrderedWeightedNetwork()  
     {
            ...
            for (int er=0; er<TotalNumberStubs/2; er++) 
            {
                ...
                if (weightedEdges) System.out.println(SEP+stubValueList[e].label+SEP+stubValueList[e].weight);
           }
            System.out.println("***                      ***");
     }
...
// --------------------------------------------------------------------------  
     ...
     /**
      * Prints information about the type of graph to standard output.
      * @param sep separation string
     */
     public void printGraphType(String sep){
        printGraphType(System.out, sep);
    }
     /**
      * Prints information about the type of graph
      * @param PS a PrintStream such as System.out
     * @param sep separation string
     */
     public void printGraphType(PrintStream PS, String sep){
        PS.println(graphTypeString(sep));
    }
       
    /** 
     * Prints very basic parameters to standard output.
     */
    public void printParametersVeryBasic()  {
     ...
     * @param sep separation string
     */
    public void printParametersVeryBasic(PrintStream PS, String sep)  {
        PS.println("Programme version "+sep+VERSION);
    ...
    public void printParametersBasic(PrintStream PS, String sep)  {
        ...
        PS.println("Number Vertices " +sep+ TotalNumberVertices+sep+"Stubs" +sep+ TotalNumberStubs+sep+"Total Weight" +sep+ this.getTotalWeight());
        if (bipartiteGraph) {
            PS.println("Number "+this.nameVertexType1+" (type 1) and "
                    +this.nameVertexType2+" (type 2) vertices"+sep
                    +numberVertexType1+sep+numberVertexType2);
        }
        PS.println("Average Out degree "+sep+calcDegreeFirstMoment()+sep+", Second Moment of D.D."+sep+calcDegreeSecondMoment());
        if (outputControl.triangleSquareOn) {
              PS.println("Total Number of Triangles "+sep+getNumberTriangles()+
                      ", global clustering coefficient "+sep+getCCGlobal());
        }
        PS.println(graphTypeString(sep));
        PS.println(stringOfChecks(sep));
        PS.println("Output: "+outputControl.getModeString(sep));
    }
    ...
    /**
     * Name for network.
     * <p>Currently uses root of number of initial input file.
...
// -------------------------------------------------------------    
    ...
    public int setNetworkOnlyFromInputFile() {
        ...
        switch (inputFileType.getNumber()) {
            ...
            case 1: {
                res = setNetworkEdgeList(inputName.getNameEnd(), directedGraph, weightedEdges, vertexlabels, labelledEdges, true, true);
                initialVertices = TotalNumberVertices;
                initialEdges = TotalNumberStubs;
                return res;
            }
            case 2: {
                res = setNetworkEdgeList(inputName.getNameEnd(), directedGraph, weightedEdges, vertexlabels, labelledEdges, false, true);
                initialVertices = TotalNumberVertices;
                initialEdges = TotalNumberStubs;
                return res;
            }
     ...
     * Then looks for <em>inputName</em><tt>inputVertices.dat</tt>.
     * This is used to read in properties from file with a header.
     * If such a file is not found then individual files are NOT used
     * as described in <tt>setVertexPropertiesFromManyFiles</tt>.
     * @return
     * @see #setVertexPropertiesFromFile(java.lang.String, java.lang.String, boolean, boolean)
     * @see #setVertexPropertiesFromManyFiles(int, int, int, boolean, boolean)
     */
    public int setNetworkFromInputFiles() {
        int res=setNetworkOnlyFromInputFile();
        //String vertexInformationFullFileName=inputName.getNameRootFullPath()+"inputVertex.dat";
         String ext="inputVertices.dat";
         String cc=timgraph.COMMENTCHARACTER;
         boolean headerOn=true;
         boolean infoOn=false;
         try{
             setVertexPropertiesFromFile(ext,cc,headerOn,infoOn);
         } catch(RuntimeException e){
             System.out.println("*** Unable to set vertex propetries, error:- "+e);
             
         }
         //    setNetworkFromManyInputFiles();
         return res;
    }

    /**
     * Sets up network and vertex values from input files using multiple files for vertices.
     * <p>Basic network file used is indicated by ending of current <tt>inputName</tt>
     * which is compared against the <tt>InputFileType</tt>.
     ...
     * @return
     * @see for single file input use {@link #setNetworkFromManyInputFiles() }
     */

    public int setNetworkFromManyInputFiles() {
      int xColumn=1;
      int yColumn=2;
      int nameColumn=1;
      boolean headerOn=false;
      boolean infoOn=false;
      return setNetworkFromManyInputFiles(xColumn, yColumn, nameColumn, headerOn, infoOn);
    }
     ...
     * @return
     */
    public int setNetworkFromManyInputFiles(int xColumn, int yColumn, int nameColumn, boolean headerOn, boolean infoOn) {

        int res=setNetworkOnlyFromInputFile();
        setVertexPropertiesFromManyFiles(xColumn, yColumn, nameColumn, headerOn, infoOn);
        return res;
    }
    /** 
     *  Set initial test graphs.      
    ...
    void setNetwork(int n)  
    {
        ...
        if (n==99){ setNetworkFromInputFiles(); return;}
       ...
       if (weightedEdges) for (int e=0; e<TotalNumberStubs; e++) stubValueList[e] = new EdgeValue(0,1);
...
// -------------------------------------------------------------
    ...
    /**
     *  Set up empty vertex properties for graph capable of holding maximumVertices.
    ...
    {
       //System.out.println("Maximum vertices and edges are " + maximumVertices + SEP + maximumStubs);
       if (maximumVertices<0) System.err.println("*** ERROR Must have positive maximum number of vertices, currently "+maximumVertices);
       if (maximumVertices<TotalNumberVertices) System.err.println("*** ERROR Cuurenty have "
               +TotalNumberVertices
               + " but current maximum number of vertices is only "+maximumVertices);
    ...
    public void setNetworkEdges()
    {
       ...
       if (weightedEdges) stubValueList = new EdgeValue[maximumStubs];
       TotalNumberStubs =0;
    }
...
//       TotalNumberStubs =0;
       ...
       if (shuffleOn)
       {
           ...
           int r=-1;
           for (int i = 0; i < N; i++) {
            r = (int) (Math.random() * N );  // allow to shuffle to self - faster than checking v!=r everytime?
            value = stubList[i];
...
// -------------------------------------------------------------    
     ...
     *@param ext is the ending on the number of file.
     *@param directed true if directed graph being input
     *@param weighted true if weighted graph being input
     ...
     *@param repeatEdgesAdd if true repeated edges weights are added if weighted and not multiedged, else ignored
     *@return flag from   input.processPajekFile
     */
    public int setNetworkEdgeList(String ext, boolean directed, boolean weighted, 
            boolean vertexLabelled , boolean edgeLabelled, boolean intList, boolean repeatEdgesAdd )
    {
       FileInput input = new FileInput(this.infoLevel);
       if (intList) return FileInput.processIntEdgeFile(this, ext, 1, 2, 
               (weighted?3:-1), (edgeLabelled?4:-1), 
               directed, vertexLabelled, (infoLevel>1)  );
       return input.processStringEdgeFile(this, ext, 1, 2, (weighted?3:-1), 
               (edgeLabelled?4:-1), directed, vertexLabelled, repeatEdgesAdd  );       
    }
...
// ---------------------------------------------------------------------------------
 ...
 public void setNetworkCubicLattice(int nVertices, int m, int xLength, int yLength, boolean periodic)
 {
   ...
   for (int z=0; z<zdim; z++)
   {
     ...
     * <br>Old indexValue of vertex is left as label
     * @param projStemName number to add to old outputName root for new graph
     * @param sgvertexList the set of indices of vertices to be retained
     * @param makeDirected true (false) if want (un)directed graph
     * @param makeLabelled true (false) if want (un)labelled graph
     * @param makeWeighted true (false) if want (un)weighted graph
     * @param makeVertexEdgeList true (false) if (don't) want vertexEdgeList to be made.
     * @param makeBipartite true (false) if want to make bipartite projection (only if input is bipartite)
     * @return subgraph
     */
    public timgraph projectSubgraph(String projStemName, TreeSet<Integer> sgvertexList,
            boolean makeDirected, boolean makeLabelled, boolean makeWeighted, 
            boolean makeVertexEdgeList, boolean makeBipartite
          ){
        int stubs=0;
        ...
        boolean setBipartite = (makeBipartite && this.isBipartite());
        int n1=0;
        int n2=0;
        for (Integer vold : sgvertexList){
            ...
            if (makeLabelled) {
                ...
                vl.setNumber(vold); // add old indexValue as number
                newtg.addVertex(vl); 
            } 
        ...
        if (setBipartite) setBipartite(n1, n2, this.getNameVerticesType1(), this.getNameVerticesType2());
        ...
        for (int e=0; e<TotalNumberStubs; e++){
            ...
            if (sgvertexList.contains(s) && sgvertexList.contains(t) ) {
                ...
                if (makeWeighted) {
                    if (weightedEdges) newtg.addEdge(ns,nt,stubValueList[e]);
...
// -------------------------------------------------------------    
     ...
     * @param name1 number of type one vertices
     * @param name2 number of type two vertices
...
// -------------------------------------------------------------    
    ...
    * @deprecated use Timgraph.algorithms.LineGraphProjector
    ...
    public timgraph makeLineGraph(int newtype, boolean makeUndirected) {
        ...
        int type=0;
        if ((newtype>0)&&(newtype<LineGraphType.lgExtensionList.length)) type = newtype;
        timgraph tg = new timgraph();
        ...
        tg.inputName.appendToNameRoot(LineGraphType.lgExtensionList[type]);
        tg.outputName.appendToNameRoot(LineGraphType.lgExtensionList[type]);
   ...
   /**
    ...
    public timgraph makeConsolidated(double minWeight, boolean makeUndirected, boolean makeLabelled, boolean makeWeighted, boolean makeVertexEdgeList)   {
        ...
        double dw=-1;
        for (int e = 0; e < TotalNumberStubs; e++) {
                if (this.weightedEdges) dw = stubValueList[e].weight; else dw=1;
                s=this.stubSourceList[e++];
                t=this.stubSourceList[e];
   ...
   /**
    ...
    public timgraph makeUnweighted(double minWeight, boolean makeUndirected, boolean makeLabelled, boolean makeVertexEdgeList)   {
        ...
        tg.maximumStubs = 0;
        for (int e = 0; e < TotalNumberStubs; e++) {
                if (stubValueList[e].weight>=minWeight) tg.maximumStubs++;
        }
        ...
        int t=-1;
        for (int e = 0; e < TotalNumberStubs; e++) {
                if (stubValueList[e].weight<minWeight) {e++; continue;}
                s=this.stubSourceList[e++];
                if (stubValueList[e].weight<minWeight) continue;
                t=this.stubSourceList[e];
...
//        if (vertex==getNumberVertices()) System.err.println("*** NOT FOUND VERTEX "+vertexName);
        ...
        boolean makeBipartite=true;
        timgraph newgraph = projectSubgraph("Ring",sgvertexList, makeUndirected, makeLabelled, makeWeighted, makeVertexEdgeList, makeBipartite) ;
//        if (tg.getNumberVertices()<20) tg.printNetwork(true);
//        newgraph.inputName.setNameRoot(number+vertexName+maxDistance);
//        newgraph.outputName.setNameRoot(number+vertexName+maxDistance);
        return newgraph;
    }
    ...
    */
    public timgraph makeEdgeSubGraph(String vertexName, boolean makeUndirected, boolean makeWeighted, boolean makeLabelled, boolean makeVertexEdgeList){
        ...
        boolean makeBipartite=isBipartite();
        timgraph newgraph = projectSubgraph("proj",sgvertexList, 
                makeUndirected, makeLabelled, makeWeighted, makeVertexEdgeList,
                makeBipartite) ;
    ...
    public TreeSet<Integer> getEdgeLabelSet(){
        ...
        int numberLabels=0;
        for (int e=0; e<TotalNumberStubs; e+=2 ){
            labelSet.add(stubValueList[e].label);
        }
        return labelSet;
    }
...
//    public HashMap<Integer, Integer> getEdgeLabelMap(int vertex)
    ...
    * to any edge in subgraph that did not have one of the labels.
     * @param vertexName number of vertex
    */
    ...
    */
    ...
    private int edgeToVertexLabel(int v, double minimumValue, boolean normaliseOn, boolean useWeights){
        ...
        for (int ei=0; ei<edgeList.size(); ei++){
            int e=edgeList.getQuick(ei);
            int c= this.stubValueList[e].label;
            double w= (useWeights?stubValueList[e].weight:1);
            s+=w;
            if (communityToIndex.containsKey(c)) { //known community
                int i = communityToIndex.get(c);
                strengthInCommunity.setQuick(i,strengthInCommunity.getQuick(i)+w);
...
//          if (k>maximum) maximum=k;
  ...
  /**
     ...
     * @param filenamecomplete full number including directory and extension
     * @param cc comment characters put at the start of every line
     * @param normalise a boolean parameter to swicth on normalisation
     */
