 ...
 * @author time
 */
public class MakeLineGraphCommunity {
    ...
    public MakeLineGraphCommunity(timgraph newtg){tg=newtg;}
    
    final static String [] ordinalsList={"First","Second","Third","Fourth","Fifth","Sixth","Seventh"};
    ...
    static public void usage(PrintStream PS){
     ...
     PS.println(ordinalsList[++nnn]+" "+arg+" argument is the network number for the setup routine OR ? for this help");
     PS.println(ordinalsList[++nnn]+" "+arg+" argument is the method used to find communities, 0=Louvain/Greedy, 1= Simulated Annealing");
     PS.println(ordinalsList[++nnn]+" "+arg+" argument is type of line graph to use 0=C, 1=Ctilde, 2=D, 3=Dtilde, 4=E, 5=Etilde");
     PS.println(ordinalsList[++nnn]+" "+arg+" argument is definition of quality to use, 0=Q(A), 1=Q(A"+arg+"A-A)");
     PS.println(ordinalsList[++nnn]+" "+arg+" argument is type of quality class to use, 0=basic (dense matrix), 1=sparse matrix, 2=minimal memory ");
     PS.println(ordinalsList[++nnn]+" "+arg+" argument is value of lambda, scaling parameter of null model.");
     ...
     * the rest of this argument is for this class.
     * If arguments exists but the first has no ':' then network is set up from file read in using the arguments
     ...
     * <p>Third ':' argument is type of quality class to use, 0=basic (dense matrix), 1=sparse matrix (best for directed), 2=minimal memory (best for undirected)");
     * <p>Fourth ':' argument is type of line graph to use 0=C, 1=Ctilde, 2=D, 3=Dtilde, 4=E, 5=Etilde
    ...
    { 
      System.out.println("LineGraphCommunities Arguments :method :qualitydef :qualityclass :linegraphtype :lambda");
      int ano=-1;
      ...
      // qualityType 0=basic (dense matrix), 1=sparse matrix (for directed graphs), 2=minimal memory (for undirected graphs)
      int qualityType=1;
      ano++;
      ...
      int lgmethod = 4; // 0=Line Graph (C),
                      ...
                      // 4 = Strength Weighted Line Graph, E(G) 
                      // 5 = Strength Weighted Line Graph with self-loops, Etilde(G)
      ano++;
...
//      String basicFileName="BowTieWD"; //BowTieW";
//      String fileEnding="inputELS.dat"; 
//      String inputDataDir = "input/BowTie";
//      String outputDataDir = "output/BowTie";
//      String [] aList = { "-gvet", "-gdt", "-gewt", "-gvlt", "-gbf", 
//          "-fin"+basicFileName+fileEnding, "-fis"+inputDataDir, "-fos"+outputDataDir, 
//          "-gn99",  "-e0", "-o23", "-xi0"};
      String basicFileName="UKCensus01EWDistSymmCommFlow"; //BowTieW";
      String fileEnding="inputELS.dat"; 
      String dataDir = "/DATA/UKCensus/";
      String inputDataDir = "input/";
      String outputDataDir = "output/";
      String [] aList = { "-gvet", "-gdf", "-gewt", "-gvlt", "-gbf", 
          "-fin"+basicFileName+fileEnding, "-fad"+dataDir, //"-fis"+inputDataDir, "-fos"+outputDataDir, 
          "-gn99",  "-e0", "-o23", "-xi0"};
...
//      tg = SetUpNetwork.setUpNetwork(n, args);
     ...
     tg.parseParam(aList);
     if (tg.isDirected()){
         if (qualityType>1) throw new RuntimeException("Input graph is weighted and/or directed, needs quailty type 0 or 1 not "+qualityType);
     }
...
//      if ((n>=140)&& (n<150)) EP2VCprojOn=true;
     process(tg, method,lgmethod,qdef,qualityType,lambda, EP2VCprojOn,incidenceGraphOn);
    }

    static public void process(timgraph tg,int method,int lgmethod,int qdef,int qualityType,double lambda,
            boolean EP2VCprojOn,boolean incidenceGraphOn){
      ...
      boolean linegraphOutputOn=false;
      boolean printnn=false;
      OutputMode outputControl=null;
      fo.informationGeneral("", SEP, 
              vertexListOn, printnn,
...
//      vertexPartitionGraphML(tg, louvainvp1);
      ...
      if (linegraphOutputOn) folg.printEdges(true,true,true,true,true);
...
//      }
      ...
      System.out.println("*** Line Graph partition of type "
              +QualityType.qualityLongName[qualityType]+" "
              +QualitySparse.QdefinitionString[qdef]
              +", lambda="+lambda);    
...
//        if (nextbit) {
            ...
            System.out.println("*** Vertex partition of type "+QualitySparse.QdefinitionString[qdef]+", lambda="+lambda);
...
//
    ...
    static public void analyseGraph(timgraph tg, int method, int lgmethod, int qdef, int qualityType, double lambda){
      ...
      VertexPartition louvainvp = VertexPartition.calculate(tg, qdef, qualityType, lambda, method, infoLevel); 
      //testGraphViz(bestvp);
      MakeLineGraphCommunity.vertexPartitionGraphML(tg, louvainvp);
      ...
      fo.printEdgeCommunityStats(true, true, splitBipartite, outputType1);
            ...
            MakeLineGraphCommunity.edgePartitionGraphML(tg, louvainlgvp);
            MakeLineGraphCommunity.vertexEdgePartitionGraphML(tg, louvainvp, louvainlgvp);
    ...
    public static void doEP2VC(FileOutput fo, timgraph tg, VertexPartition lgvp, int qdef, double newlambda, int qualityType){
        VertexCommunity vc = new VertexCommunity(tg,lgvp, qdef, (tg.isDirected()?4:0), qualityType, newlambda);
        vc.calcQuality();
...
//    }
    ...
    public static void incidenceGraph(int vpmethod) {
        ...
        foig.graphML();
        foig.edgeListSimple();
        foig.printVertices("", "\t", null, true, true, true, false, false, 
                ig.isDirected(), 
