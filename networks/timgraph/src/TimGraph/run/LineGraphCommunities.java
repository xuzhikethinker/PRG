...
public class LineGraphCommunities {
    ...
    { 
      ...
      fo.informationGeneral("", SEP, 
              vertexListOn, printNearestNeighbours, edgeListOn, graphMLOn, outputControl);
...
//      testLambda(method);
//      }
      int nullModel=(lineGraphtg.isDirected()?4:0);
      System.out.println("*** Line Graph partition of type "+QualitySparse.QdefinitionString[qdef]+", lambda="+lambda);    
      louvainep = VertexPartition.calculate(lineGraphtg, qdef, nullModel, qualityType, lambda, method, infoLevel);
      louvainep.setName(louvainep.getName() + LineGraphProjector.lgExtensionList[lgmethod]);
        ...
        if (nextbit) {
            ...
            fo.printVertices("", SEP, false, true, null,  false);
            ...
            VertexPartition louvainvp = VertexPartition.calculate(tg, qdef, nullModel,
                    qualityType, lambda, method, infoLevel); 
      ...
      if ((n>=140)&& (n<150)) EP2VCprojOn=true;
        if (EP2VCprojOn) {
           ...
           doEP2VC(fo, tg, louvainep, qdef , nullModel, lambda, qualityType);
      }
...
//      folg.informationVertexPartition(louvainlgvp,"",SEP,true);
     ...
     */
    static public void analyseGraph(timgraph tg, int method, 
            int lgmethod, int qdef, int nullModel, int qualityType, double lambda){
      ...
      VertexPartition louvainlgvp = VertexPartition.calculate(lg, qdef, nullModel,
                    qualityType, lambda, method, infoLevel);
     ...
     * @param qualityType
     */
    public static void doEP2VC(FileOutput fo, timgraph tg, VertexPartition lgvp, int qdef, 
            int nullModel, double newlambda, int qualityType){
        VertexCommunity vc = new VertexCommunity(tg,lgvp, qdef, nullModel,
                    qualityType, newlambda);
        vc.calcQuality();
     ...
     * @param qualityType
     */
    public static void doEP2VCandVP(FileOutput fo, timgraph tg, VertexPartition lgvp, 
            int qdef, int nullModel, double newlambda, int qualityType){
        VertexCommunity vc = new VertexCommunity(tg,lgvp, qdef, nullModel,
                    qualityType, newlambda);
        vc.calcQuality();
        fo.informationVertexCommunity(vc, "", SEP, true);
...
//    }
    ...
    public static void incidenceGraph(int vpmethod) {
        ...
        foig.printVertices("", "\t", null, true, true, true, false, false, 
                ig.isDirected(), false, false, false, false, false, true);

        VertexPartition iglouvainvp = VertexPartition.calculate(ig, 0, 
                    vpmethod, infoLevel);
