...
public class FileOutput {
    ...
    /**
     * Output a collection to a file as single column
     * @param coll collection to be output
     * @param headerLines array of line to output first 
     * @param sep separation string
     * @param infoOn if true gives information on screen
     */
    static public void Collection(Collection coll, String fileName, 
            String[] headerLines, String sep, boolean infoOn){
        PrintStream PS;
        FileOutputStream fout;
        if (infoOn) System.out.println("Writing file of map to "+ fileName);
            try {
            fout = new FileOutputStream(fileName);
            PS = new PrintStream(fout);
            if (headerLines!=null) for (int line=0; line<headerLines.length; line++) PS.println(headerLines[line]);

            for (Object k: coll) PS.println(k);
            try{ fout.close ();   
               } catch (IOException e) { throw new RuntimeException("*** File Error with " +fileName+" "+e.getMessage());}
    ...
    /**
     * Writes out matrix as edge list.
     * If entry in matrix is zero then no edge output.
     * If indexToLabel array is given then labels are used for source and target vertices.,
     * otherwise integer index is given.
     * @param edgeFile full name of file for output
     * @param headerLines array, each being a line to put at start
     * @param indexToLabel index to label array, if dimension dim then names used, else ignored and indices used
     * @param matrix values for edges
     * @param dim dimension of array
     * @param sep separation character used on each line such as tab
     * @param infoOn true if want information printed
     */
    static public void matrixAsSparseEdgeList(String edgeFile, String[] headerLines,
            String [] indexToLabel, float [][] matrix, int dim,
            String sep, boolean infoOn){
        PrintStream PS;
        FileOutputStream fout;
        boolean useLabel=true;
        if ((indexToLabel==null) || ( indexToLabel.length!=dim)) useLabel=false;
        if (infoOn) System.out.println("Writing file of matrix to "+ edgeFile+" using "+(useLabel?"labels":"indices"));
            try {
            fout = new FileOutputStream(edgeFile);
            PS = new PrintStream(fout);
            if (headerLines!=null) for (int line=0; line<headerLines.length; line++) PS.println(headerLines[line]);
            String slabel, tlabel;
            for (int s=0; s<dim; s++){
              if (useLabel) slabel=indexToLabel[s];
              else slabel=Integer.toString(s);
              for (int t=0; t<dim; t++){
                if (matrix[s][t]==0) continue;
                if (useLabel) tlabel=indexToLabel[t];
                else tlabel=Integer.toString(t);
                PS.println(slabel+sep+tlabel+sep+matrix[s][t]);
              }
            }
            try{ fout.close ();
               } catch (IOException e) { throw new RuntimeException("*** File Error with " +edgeFile+" "+e.getMessage());}

        } catch (FileNotFoundException e) {
            throw new RuntimeException("*** Error opening output file "+edgeFile+" "+e.getMessage());
        }
        if (infoOn) System.out.println("Finished writing matrix of map to "+ edgeFile);
    }
...
//            else PS.println();
   ...
   /**
    ...
    public void informationIncidenceVertexPartition(VertexPartition ip, String cc, String sep, boolean listPartitionOn){
        ...
        if (infoLevel>-2) {
            System.out.println("Writing incidence matrix vertex partition information file to "+ fileName.getFullFileName());
        }
            try {
            fout = new FileOutputStream(fileName.getFullFileName());
            PS = new PrintStream(fout);
            ...
            if (infoLevel>-2) {
                    System.out.println("Finished writing vertex information file to "+ fileName.getFullFileName());
                }
            try{ fout.close ();   
               } catch (IOException e) { System.err.println("*** File Error with " +fileName.getFullFileName()+" "+e.getMessage());}
    ...
    public void printVertices(String cc, String sep, boolean infoOn, boolean headerOn,
        ...
        try {
            ...
            if (infoLevel>-2) {
                if (simpleListOn) {
                    System.out.println("Finished writing simple list of vertex - partition pairs to file "+ fileName.getFullFileName());
                }
                else {
                    System.out.println("Finished writing vertices and their vertex partition information to file "+ fileName.getFullFileName());
                }
            }
            try{ fout.close ();   
               } catch (IOException e) { System.out.println("File Error");}
            ...
            boolean printNearestNeighbours )
    {
            ...
            printVertices(cc, sep, vp, printTriangles, printSquares,
                    printName,printNumber,  printPosition, tg.isBipartite(),
    ...
    {
        informationGeneral(cc, sep,  true, false, true, true, null);
    }
            ...
            OutputMode outputControl)
    {
        ...
        if (tg.isWeighted() && tg.isVertexLabelled()) {
            tg.calcStrength();
        }
        ...
        try{
            if (om.degreeDistributionOn){
              ...
              double lbratio=1.1;
              if (tg.getNumberVertices()>100) {
                    tg.FileOutputLogBinnedDegreeDistribution(cc, lbratio, infoOn, headersOn);
                }
            }
        ...
        try{
            if (om.adjacencyFileOn) {
                adjacencyMatrix(timgraph.SEP,true,true);
            }
        ...
        if (om.triangleSquareOn){
            printTriangles=true;
            printSquares=true;
        }
            
        if (vertexListOn) {
            printVertices(cc, sep, vp,
                    printTriangles, printSquares,
                    printNearestNeighbours );
        }
