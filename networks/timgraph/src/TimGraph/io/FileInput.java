...
//            target = vertexLabelToIndex.get(edgeLL.get(e));
            ...
            if (tg.isWeighted()) {
                if (weightsOn) {
                    weight=weightLL.get(e/2);
                } else {
                    weight =1.0;
                }
                if (labelsOn) {
                    label=labelLL.get(e/2);
                } else {
                    label =0;
                }
                if (multiEdges) {
                    tg.increaseEdgeWeight(source,target,weight);
                }
                else {
                ...
                tg.increaseEdgeWeight(source,target,1.0);
            }
                 else {
...
//            String target ="NOTSET";
   ...
   // ***************************************************************************
    ...
    public int processStringEdgeFile(timgraph tg, String ext, 
            int columnSource, int columnTarget, int columnWeight,  int columnLabel, 
            boolean directed, boolean vertexLabelled, boolean repeatEdgesAdd )
    {
        boolean weightsOn = true;
        if (columnWeight<0) weightsOn = false; 
        boolean labelsOn = true;
        ...
        int maxWarning=20;
        res=0;
        //int locale =-1; // local edge (to target) index for source vertex
        for (int e=0; e < edgeLL.size(); e++)
        {
            source = vertexLabelToIndex.get(edgeLL.get(e++));
            target = vertexLabelToIndex.get(edgeLL.get(e));
            ...
            if (tg.isWeighted()) {
                ...
                //locale = -1;
                //if (!tg.isMultiEdge()) tg.getFirstEdgeLocal(source,target);
                if (tg.isMultiEdge() ) {
                    tg.addEdge(source,target,new EdgeValue(label, weight));
                    continue;
                }
                if (repeatEdgesAdd) tg.increaseEdgeWeight(source,target, weight);
                else { // repeated edge ignored
                  if (res>-maxWarning && (infoLevel>-2)) System.out.println("!!! edge "
                          +(e/2)+" is second edge from "
                          +source+"("+edgeLL.get(e-1)+") to "
                          +target+"("+edgeLL.get(e)
                          +") -> "+(repeatEdgesAdd?"added":"ignored"));
                  ...
                  if (res>-maxWarning && (infoLevel>-2)) System.out.println("!!! edge "+(e/2)
                          +" is second edge from "
                          +source+"("+edgeLL.get(e-1)+") to "
                          +target+"("+edgeLL.get(e)
                          +") -> ignored");
...
//        tg.minimumCoordinate = new Coordinate();
            ...
            boolean forceLowerCase, boolean infoOn)
    {
         ...
         * @param infoOn true if want the input lines printed out as read in
         */
    static public TreeSet<String>  getVertexNamesFromFile(String fullFileName, String cc,  
            boolean headerOn, boolean infoOn)
    {
        boolean forceLowerCase=false;
        String [] labelList = {VertexLabel.NAMELABEL};
        final int nameIndex=0;
        ArrayList<ArrayList<String>> propertyList = 
                FileInput.readStringColumnsFromFile(fullFileName,
