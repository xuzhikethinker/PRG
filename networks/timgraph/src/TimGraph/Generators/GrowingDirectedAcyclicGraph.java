 ...
 * @author time
 */
public class GrowingDirectedAcyclicGraph extends DirectedAcyclicGraph {

    final static String nameDAG="Directed Acyclic Graph";
    final static String shortNameDAG="DAG";
    ...
    * @param averageOutDegree average out degree to aim for (upper bound)
     ...
     */
     public GrowingDirectedAcyclicGraph(Random rndnew, int numberVertices, double averageOutDegree, int deltaV,
...
//            nCommunities=numberCommunities;
    ...
    public static void main(String[] args) {
     ...
     ano++;
     int initialNumberVertices=100;
     if (args.length>ano ) if (timgraph.isOtherArgument(args[ano])) initialNumberVertices=Integer.parseInt(args[ano].substring(1, args[ano].length()));
     ...
     GrowingDirectedAcyclicGraph dag = new GrowingDirectedAcyclicGraph(rndm,numberVertices, averageOutDegree, deltaV, bestFixedDegree, longestSpine, arguments);
     ...
     dag.setName();
     double avDegree=3.0;
//     GrowingAcyclicGraph.createFixedDegreeDAG(tg, initialNumberVertices,
//             avDegree, deltaV,  bestFixedDegree, longestSpine);
     dag.labelVertices();
     dag.information();
    }
...
//        //final EdgeValue ew=  new EdgeValue(1, 1.0);
//        TreeSet<Integer> nnSet;
//        for (int v=0; v<nVertices-1; v++){
//            if (bestFixedDegree) dv=Math.min(nVertices-v-1,deltaV);
