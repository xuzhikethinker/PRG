...
public class timgraph {
    ...
    /**
     * Header of vertex index column in input file
     */
    String vertexIndexLabel="Index";
    /**
     * Header of vertex index column in input file
     */
...
//    UndirectedGraph g = new UndirectedSparseGraph();
    ...
    /**
     * If defined this is a map of vertex name to global index.
     */
...
// ---------------------------------------------------------      
   ...
   * @return Vertex number to vertex global index map, null if no labels.
   */
  public void setVertexNameToIndexMap(){
      //if (!vertexlabels) throw new IllegalArgumentException("graph with labelled vertices needed to find named vertex.");
      if (!vertexlabels) {nameToIndex=null; return; }
  ...
  /**
   * Vertex name to vertex global index map.
   * @return Vertex number to vertex global index map
   */
...
// ---------------------------------------------------------      
 ...
 // ---------------------------------------------------------      
  ...
  public void setEdgeWeight(int gs, double value)  {
          edgeValuetList[gs].weight=value;
      return;
  }
   ...
   * <br>Initialises new <tt>vertexLabelList</tt> if does not exist.
   * If index is outside allowed range then warns, ignores but continues.
   * If vertex index is allowed but does not exist it is created.
   ...
   * If index list is empty or null, the vertices indexed in order they are given
   * starting from 0.
  ...
  public void setVertexPositionsFromStrings(int [] indexList,
      ...
      if (!this.vertexlabels) initialiseVertexLabels();
      int v=IUNSET;  //global index to vertex to be updated
      Double x=DUNSET;
      Double y=DUNSET;
   ...
   * <br>vertex names are in index order.
   ...
   * left unchanged.
   * @param nameList list of names as strings in index order
  ...
  public void setVertexNames(int [] indexList, ArrayList<String> nameList)  {
      ...
      if (!this.vertexlabels) initialiseVertexLabels();
      int v=IUNSET; // global index of vertex to be updated
      String name="UNSET";
      Warning warning = new Warning();
      int nSet=0;
      boolean useIndexList=true;
      ...
      for (int i=0; i<nameList.size(); i++) {
          ...
          try{
              ...
              if (this.vertexLabelList[v].hasName()) {
                  if (!vertexLabelList[v].getName().equals(name))
                      warning.nextErr(i+"th vertex, index "+v
            ...
            System.out.println("Set "+nSet+" names");
  }
         ...
         * in the index order starting from 0.  Generally if vertex doesn't
         ...
         * @param infoOn true if want the input lines printed out as read in
         */
    public void setVertexPropertiesFromFile(String ext, String cc,  boolean headerOn, boolean infoOn)
    {
        String fullFileName=inputName.getNameRootFullPath()+ext;
        ...
        if ((propertyList.get(indexIndex)!=null) && (!propertyList.get(indexIndex).isEmpty())) {
            // vertex index given so use to map onto properties
            indexList = new int[propertyList.get(indexIndex).size()];
            int r=0; // data row in file
            for (String s: propertyList.get(indexIndex)) try{
                indexList[r++]=Integer.parseInt(s);
            } catch (RuntimeException e){
                indexList[r++]=IUNSET;
                warning.nextErr("Data row "+r+" index "+s+" not integer");
                }
        } // eo if (propertyList.get(indexIndex).size()>0)
        else{ // if no index use given names
            if ((propertyList.get(nameIndex)!=null) && (!propertyList.get(nameIndex).isEmpty())){
                // no index given so use given names
                if (nameToIndex==null) setVertexNameToIndexMap();
                ...
                for (String s: propertyList.get(nameIndex)) {
                    try{
                       index=nameToIndex.get(s);
                    } catch (RuntimeException e){
                       index=newVertexNumber++; // assign next available new vertex number
                       warning.nextOut("Data row "+r+" name "+s+" unknown");
                    }
                    indexList[r++]=index;
                }// eo for s
            } // if (propertyList.get(nameIndex).size()>0)
            else{ // no index and no name, so treat as being in numerical order
                indexList=null;
            }
        }
...
//                        throw new IllegalArgumentException("*** Argument "+v+" is too short "+argument);
//                    }
...
//    }
     ...
     * Then looks for <em>inputName</em><tt>inputVertex.dat</tt>.
