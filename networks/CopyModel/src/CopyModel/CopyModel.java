...
//                printArtefacts(System.out, " ",false);
        ...
        if (outputControl.allRunStatisticsOn) {
            String ss = "t"+event*numberRewiringsPerEvent+shortDescription(5);
            if (artDDStatsOn) FileOutputTimeDependentData("", ss+"art", artDDStatsArr, false);
            if (this.artDDStatsTemplate.Y>0) FileOutputTurnoverTable(COMMENTSTRING, "t"+event*numberRewiringsPerEvent, false);
            if (inflDDStatsOn) FileOutputTimeDependentData("", ss+"infl", inflDDStatsArr, false);
        }
    } //eo rungeneralmodel


    /**
     * Constructs string without spaces representing key parameters.
     * Used for filenames.
     * @param decimalPlaces number of decimal places
     * @return short string containing key parameters
     */
    private String shortDescription(int decimalPlaces){
        String s;
        s="mr"+rewireMode.get()+"ppm"+ppMode+"prm"+prMode+"pr"+TimUtilities.FractionUtilities.fractionToString(prob.pRand,decimalPlaces);
        return s;
    }
...
// STATISTICAL CALCULATIONS
    ...
    private int warningNumber(){return (infolevel>0?10:0);}
// -----------------------------------------------------------------------       
    /**
    ...
    void calcArtefactWeightDistribution()  {
        ...
        weightArtCont =0;
        weightArt=new StatisticalQuantity(warningNumber());
...
// -----------------------------------------------------------------------
    ...
    void calcArtefactDegreeDistribution()  {
        ...
        degreeArtCont =0;
        artDegree=new StatisticalQuantity(warningNumber());
...
// -----------------------------------------------------------------------
    ...
    void calcInfluenceDegreeDistribution()  {
        ...
        degreeInflCont =0;
        degreeInfl=new StatisticalQuantity(warningNumber());
     ...
     */
        ...
        void printTurnoverTable(PrintStream PS, String cc, boolean giveGeneralInformation)  
        {
            ...
            for (int y=0; y<artefactDegreeRank.Y; y++) {
                PS.print((y+1));
                StatisticalQuantity ystats = new StatisticalQuantity(warningNumber());
