...
public class JournalWithASJC extends Journal  {
     ...
     * @param title journal title
     * @param ISSN ISSN number, perhaps with leading zeros missing
     * @param did data source journal id (scopus source id)
     * @param classString list of class numbers separated by classSep character
     * @param classSep regex separation string e.g. escaped tab \\t
     */
    public JournalWithASJC(String title, String ISSN, String did, String classString, String classSep){
        this.title=title;
        this.ISSN=ISSN;
        this.did=did;
        setASJCNumbers(classString, classSep);
    ...
    public ArrayList<Integer> getASJCList(){return ASJCList;}

    /**
     * Test to see if has ASJC code.
     * @param ASJC code test
     * @return true if code matches one in the list.
     */
    public boolean hasASJC(Integer ASJC){return (ASJCList==null?false:ASJCList.contains(ASJC));}
