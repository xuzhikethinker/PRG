...
//            column = line.split("\\t+"); // split at every tab
     ...
     * @return list of journals found.
     */
    public TreeSet<JournalWithASJC> readJournalWithASJCData(String fullFileName, int infoLevel){
        TextReader tr = ProcessScopusJournalLists.openFile(fullFileName);
        if (tr==null) return null;
        if (infoLevel>-2) System.out.println("Starting to read list of strings from " + fullFileName);
        ArrayList<String> words = new ArrayList();
        TreeSet<JournalWithASJC> journalList = new TreeSet();

        String [] labelList ={JournalTitleLabel, ISSNLabel, ASJCLabel};
        int rowNumber=0;
        try {
            String [] column;
            // first find header row and identify columns needed
...
//            column = line.split("\\t+"); // split at every tab
            ...
            while (tr.eof() == false) {
                ...
                journal = new JournalWithASJC(title, ISSN, ASJC, "\\D+"); // split at characters that are not digits
                journalList.add(journal);
    ...
    public static int [] testLabelRow(String [] columns, String [] labelList){
        ...
        for(int c=0; c<labelList.length; c++){
                columnIndex[c] =ProcessSinglePublicationCSVList.findColumn(columns, labelList[c], false);
