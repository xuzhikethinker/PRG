...
# log is natural logarithm
 ...
 Etilde = pr*Ebar;
 cest <- 1-Ktilde-Ebar;
 startlist <- list(a=Ktilde, c=cest);
 lowerlist <- list(a=Ktilde*1e-2, c=cest*100)
 upperlist <- list(a=Ktilde*100, c=cest*1e-3 )
 b <- -kmax
...
#fitres <- nls( logp ~ hypergfpdf(k,a,b,c), dataf, startlist, lower=lowerlist, upper=upperlist)
fitres <- nls( logp ~ hypergfpdf(k,a,b,c), dataf, startlist)
}
