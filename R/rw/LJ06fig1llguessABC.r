...
# these a,c values were derived from excel spread sheet using the guess pr fits.
aguesslist=c(329.676, 7.9600,	1.1291,	0.2089,	0.0573,	0.0242)
cguesslist=c(-32835.99,	-986.04,	-309.78,	-218.68,	-203.68,	-200.39)
...
kmax <- Nm1value
ifirst <- 1;
ilast <- length(aguesslist);

source("SimonData.r");
plot(SimonDataList[[ifirst]],   type="p", log="xy" ,  col=colourlist[ifirst], pch=ifirst, xlim=c(kmin,kmax), ylim=c(1e-9,1), xlab="degree k", ylab="p(k)", main="Laird and Jensen EPL 2006 Fig 1 (ll, abc Guess)")
...
#p1llguess=plot(ddpdfabc(kmin,kmax,alist[ifirst],-Nm1value,clist[ifirst]), type="l", log="xy" ,  col=colourlist[i], ylim=c(1e-9,1), xlab="degree k", ylab="p(k)", main="Laird and Jensen EPL 2006 Fig 1 (ll, abc Guess)");
