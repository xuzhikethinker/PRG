...
print(paste("Working with",graphTypeName,"graphs from files",dataFileName,paramFileName), quote=FALSE)
...
normmax=normest+1    #normest*1000; #1.0/log(k2min);
 ...
 #if (alg>0) 
 fitres <- nls( logn ~ normfit-gfit*log((k+k1fit))-zfit*log(1-(k/k2fit)) , dataFit, startlist, lower=lowerlist, upper=upperlist, algorithm="port")
...
# Now plot on screen
if (OSWindows) windows() else quartz()
plot(x=NULL,  y=NULL, type="n", log="x", xlim=c(xMin,xMax) , ylim=c(yMin,yMax) , main=plotName, sub="(Toby data)", xlab="k/(<k>-6.6)", ylab="log(n(k)*p*(<k>-6.6))")
for (iii in 1:length(ResultsList)) {
