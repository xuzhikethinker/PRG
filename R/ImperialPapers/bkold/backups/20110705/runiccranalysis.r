...
yyy <- yearList[[iii]]
lst  <-iccranalysis(rootName, cNotCoverR, normalise, facultyNotDept, yyy, yyy, numberParameters, parameterAnalysisOn, plotsOn, OSWindows, screenOn, pdfPlotOn, epsPlotOn,minCoverMeanC) 
...
if(numberParameters==3){
    ...
    for (iii in 1:length(yearList)){
        for(jjj in 1:length(unitList)){
            ...
            resultArray[((iii-1)*length(unitList)+jjj),1] <- ""
            }
            resultArray[((iii-1)*length(unitList)+jjj),2] <-  sub("_"," ",unitList[[jjj]])
...
if(texTablesOn){
    ...
    if(numberParameters==3){
        ...
        cat("\nYear & Faculty & $N_p$ & $c_0$ & $\\sigma^{2}$ & residual error/df \\\\ \\hline");
    }
    for (iii in 1:length(yearList)){
        for(jjj in 1:length(unitList)){
            if(jjj==1){
            ...
            sigma2 = round(sigmaValueList[[iii]][jjj]*sigmaValueList[[iii]][jjj],digits=1);
            sigma2err = round(10*2*sigmaValueList[[iii]][jjj]*sigmaErrorList[[iii]][jjj],digits=0);
            if(numberParameters==3){
                cat(paste("\n&",sub("_"," ",unitList[[jjj]]),
                    "&", noOfPapersList[[iii]][jjj],
                    "&", round(c0ValList[[iii]][jjj],digits=2),
                    "&", paste(sigma2,"(",sigma2err,")",sep=""),
                    "&", paste(round(dmuValueList[[iii]][jjj],digits=2),"(",round(100*dmuErrorList[[iii]][jjj],digits=0),")",sep=""),
                    "&", round(stdErrpDofList[[iii]][jjj],digits=1),"\\\\"))
