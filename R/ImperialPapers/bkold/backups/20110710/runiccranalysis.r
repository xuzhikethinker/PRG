...
yyy <- yearList[[iii]]
lst  <-iccranalysis(rootName, cNotCoverR, normalise, facultyNotDept, yyy, yyy+binnedYears-1, numberParameters, parameterAnalysisOn, plotsOn, OSWindows, screenOn, pdfPlotOn, epsPlotOn,minCoverMeanC) 
...
unitList <- lst$unitList #gsub("Department","Dept.",lst$unitList)
yValueList <- list()
xshift <- 0.2
...
if (!cNotCoverR & (numberParameters<2)) {
  ...
  sigma2Limits <- c(0.8,2.0)
  legendPosition <-"topright"
}
if (!cNotCoverR & (numberParameters>2)) {
  sigmaLimits <- c(0.9,1.3)
  sigma2Limits <- c(0.6,1.6)
  legendPosition <-"topright"
}
