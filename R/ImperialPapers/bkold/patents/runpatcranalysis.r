...
source("patcranalysis.r")
source("calcZCorrelations.r")
...
minYear=1980 #1996 for depts #1997 for facs #1997 for arXiv
maxYear=1985 #2007           #2007          #2004 
...
if(calcZCorrel){
calcZCorrelations(rootName,  catNotScat, arXiv,minYear, maxYear,plotsOn =TRUE,
...
{ 
    yyy <- yearList[[iii]] 
    lst <- patcranalysis(rootName, cNotCoverR, normalise, catNotScat, allData, yyy, yyy+binnedYears-1, numberParameters, parameterAnalysisOn, plotsOn, OSWindows, screenOn, pdfPlotOn, epsPlotOn,minCoverMeanC) 
...
for (uuu in 1:length(unitList)){
  ...
  plotCI(yearList+yearoffset+(uuu-(length(unitList)+1)/2)*xshift,yValueList,errorList, add=addValue, 
         ...
         pch=uuu, col=colourList[uuu], cex=1.2,
         main = titleString , 
         sub = NULL, xlab = "Year", ylab = expression(mu+sigma^2/2),
         xaxp=c(minYear,maxYear,maxYear-minYear)
         )
