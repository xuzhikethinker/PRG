...
if(dataSourceIndex==1){
  if (facultyNotDept){ # IC Faculty files
       ...
       minYear=1997 
       maxYear=2005##WAS 2007 ages ago
       binnedYears=3
       skipYears=1
    }  
}
...
if(dataSourceIndex==4){ # EBRP phys files
       minYear=2002 
       maxYear=2010 #2010 was 2006
       binnedYears=1
       skipYears=4 # only every 4th year available
  }
...
if(texTablesOn){
    ...
    sink(texFileName)
    if(cNotCoverR){
        cat("\\begin{tabular}{|l|l|l|l|l|l|l|l|l|l|l|l|l|}")
    }
    else{
        cat("\\begin{tabular}{|l|l|l|l|l|l|l|l|l|l|l|l|l|}")
    }
	...
	if(dataSourceIndex==2){ # arXiv files
		subgroup <- "ArXiv Subgroup"
	}
	if(dataSourceIndex==3){ # EBRP files
		subgroup <- "EBRP Subgroup"	
	}
	if(dataSourceIndex==4){ # EBRP phys files
		subgroup <- "EBRP Physics Subgroup"
	}
    ...
    if(numberParameters==3){
        if(cNotCoverR){
			...
			& $d\\mu$ & err.$d\\mu$ 
			& $dA$ & err.$dA$ 
			& $(1 + dA)$exp$(d\\mu)$ & err.$(1 + dA)$exp$(d\\mu)$ 
			& res. err.r/d.o.f.  
			\\\\ \\hline")) # '# this one and un# the other one'
        }
        else{
            #cat("\nYear & Faculty & $N_p$ & $\\langle c_r \\rangle$ & $\\langle r \\rangle$ & $\\sigma^{2}$ & $\\mu + \\frac{\\sigma^{2}}{2}$ & $dA$ &  res. err./d.o.f.
			#\\\\ \\hline")
            cat(paste("\nYear & ", subgroup ," & $N_p$ 
			& $\\langle c_r \\rangle$ & $\\langle r \\rangle$ 
			& $\\sigma^{2}$ & err.$\\sigma^2$ 
			& $d\\mu$ & err.$d\\mu$ 
			& $dA$ & err.$dA$ 
			& $(1 + dA)$exp$(d\\mu)$ & err.$(1 + dA)$exp$(d\\mu)$ 
        ...
        if(cNotCoverR){
            #cat("\nYear & Faculty & $N_p$ & $c_0$ & $\\sigma^{2}$ &  res. err./d.o.f.  & Bins & $\\chi^2/\\mathrm{d.o.f.}$ 
			#\\\\ \\hline");
            cat(paste("\nYear & ", subgroup, " & $N_p$ 
			& $c_0$ 
			& $\\sigma^{2}$ & err.$\\sigma^2$ 
            ...
            #cat("\nYear & Faculty & $N_p$ & $\\langle c_r \\rangle$ & $\\langle r \\rangle$ & $\\sigma^{2}$ & res. err./d.o.f. & Bins & $\\chi^2/\\mathrm{d.o.f.}$ 
			#\\\\ \\hline");
            cat(paste("\nYear & ", subgroup, " & $N_p$ 
			& $\\langle c_r \\rangle$ & $\\langle r \\rangle$ 
			& $\\sigma^{2}$ & err.$\\sigma^2$ 
        ...
        for(jjj in 1:length(unitList)){
            if(jjj==1){
                if(binnedYears==1){
                    ...
                    if(iii==length(yearList)){
                        ...
                        cat(paste("\n\\multirow{",length(unitList),"}{*}{",yearList[[iii]],"}",sep=""))# YOUR LINE SO THAT DATES ARE CORRECT I.E. 2007 NOT 2007-2007
						#cat(paste("\n\\multirow{",length(unitList),"}{*}{",yearList[[iii]],"-",yearList[[iii+1]]-1,"}",sep="")) # BENS LINE (ABOVE)
                    }
                }
            }
			
            sigma2 = format(sigmaValueList[[iii]][jjj]*sigmaValueList[[iii]][jjj],digits=2,ns=2);
            sigma2err = round(2*sigmaValueList[[iii]][jjj]*sigmaErrorList[[iii]][jjj],digits=2); #Is this correct? i.e. x 100??
			shouldBeOne = format((1+ dAValueList[[iii]][jjj])*exp(dmuValueList[[iii]][jjj]), digits =2, ns =2); #Is this correct? use dmu or mu + sigma^2/2?
			shouldBeOneError = round(sqrt(exp(2*dmuValueList[[iii]][jjj])*dAErrorList[[iii]][jjj]*dAErrorList[[iii]][jjj] + (1+dAValueList[[iii]][jjj])*(1+dAValueList[[iii]][jjj])*exp(2*dmuValueList[[iii]][jjj])*dmuErrorList[[iii]][jjj]*dmuErrorList[[iii]][jjj]),digits=2); #Is this correct? i.e. x 100/10?? use dmu or mu + sigma^2/2 for error?
            s = "";
            if(cNotCoverR){
            ...
            if(numberParameters==3){
                cat(paste("\n&",gsub("_"," ",unitList[[jjj]]),
                    ...
                    s,
                    "&", sigma2, #get rid of 'paste' and brackets
					"&", sigma2err, #get rid of 'paste' and brackets
                    "&", paste(format(dmuValueList[[iii]][jjj],digits=1,ns=1)),
					"&", round(dmuErrorList[[iii]][jjj],digits=2),
					"&", paste(format(dAValueList[[iii]][jjj],digits=1,ns=1)),
					"&", round(dAErrorList[[iii]][jjj],digits=2), 
					"&", paste(shouldBeOne),
					"&", paste(shouldBeOneError),
                    "&", format(stdErrpDofList[[iii]][jjj],digits=2,ns=2), 
					
					# INCLUDE IF YOU WANT TO CALC (1+dA)exp(dmu) AND THE ERROR NB. NOT SURE IF CORRECT!
					# '# above and un# below'
					#"&", paste(shouldBeOne, sep=""), # INCLUDE IF YOU WANT TO CALC (1+dA)exp(dmu) NB. NOT SURE IF CORRECT!
					"\\\\"))
            }else{
                cat(paste("\n&",gsub("_"," ",unitList[[jjj]]),
					"&", noOfPapersList[[iii]][jjj],
					s,
                    "&", paste(sigma2), #get rid of 'paste' and brackets
					"&", paste(sigma2err), #get rid of 'paste' and brackets
					"&", paste(shouldBeOne),
					"&", paste(shouldBeOneError),		
					"&", format(stdErrpDofList[[iii]][jjj],digits=2,ns=2),					
					...
					#"&", paste(shouldBeOne,"(",shouldBeOneError,")",sep=""), # INCLUDE IF YOU WANT TO CALC (1+dA)exp(dmu) AND THE ERROR NB. NOT SURE IF CORRECT!
					#"&", paste(shouldBeOne, sep=""), # INCLUDE IF YOU WANT TO CALC (1+dA)exp(dmu) NB. NOT SURE IF CORRECT!
					"\\\\"))
            }
...
unitSigma2Averages <- mat.or.vec(length(unitList),1)
unitSigma2Errors <- mat.or.vec(length(unitList),1)
unitdmuAverages <- mat.or.vec(length(unitList),1)###################################SOPHIA BELOW
unitdmuErrors <- mat.or.vec(length(unitList),1)
unitdAAverages <- mat.or.vec(length(unitList),1)
unitdAErrors <- mat.or.vec(length(unitList),1)
...
#create year averages for subject areas for sigma^2
for (iii in 1:length(yearList)){
    for(jjj in 1:length(unitList)){
        unitSigma2Averages[[jjj]] <- unitSigma2Averages[[jjj]] + sigmaValueList[[iii]][jjj] * sigmaValueList[[iii]][jjj]
        unitSigma2Errors[[jjj]] <- unitSigma2Errors[[jjj]] + 2 * sigmaValueList[[iii]][jjj] * sigmaErrorList[[iii]][jjj]
    }
}

#create year averages for subject areas for dmu#####################################SOPHIA BELOW
for (iii in 1:length(yearList)){
    for(jjj in 1:length(unitList)){
        unitdmuAverages[[jjj]] <- unitdmuAverages[[jjj]] + dmuValueList[[iii]][jjj]
        unitdmuErrors[[jjj]] <- unitdmuErrors[[jjj]] + dmuErrorList[[iii]][jjj] 
    }
}

#create year averages for subject areas for dA######################################SOPHIA BWLOW
for (iii in 1:length(yearList)){
    for(jjj in 1:length(unitList)){
        unitdAAverages[[jjj]] <- unitdAAverages[[jjj]] + dAValueList[[iii]][jjj]
        unitdAErrors[[jjj]] <- unitdAErrors[[jjj]] + dAErrorList[[iii]][jjj] 
    }
}

#print(paste("Mean Chi2 = ",meanChi2))
unitSigma2Averages <- unitSigma2Averages / length(yearList)  # i.e. sigma^2 av (per year)
unitSigma2Errors <- unitSigma2Errors / length(yearList) 
unitdmuAverages <- unitdmuAverages / length(yearList) ##############################SOPHIA BELOW
unitdmuErrors <- unitdmuErrors / length(yearList) 
unitdAAverages <- unitdAAverages / length(yearList) 
unitdAErrors <- unitdAErrors / length(yearList) 

#print(paste("Overall average = ",format(mean(unitSigma2Averages),digits=2,ns=2),"+/-",format(mean(unitSigma2Errors),digits=2,ns=2)))
for(jjj in 1:length(unitList)){
    print(paste(unitList[[jjj]]," sigma2 = ",format(unitSigma2Averages[[jjj]],digits=2,ns=2), "+/-",format(unitSigma2Errors[[jjj]],digits=2, ns = 2)))
}  
for(jjj in 1:length(unitList)){
    print(paste(unitList[[jjj]]," dmu = ",format(unitdmuAverages[[jjj]],digits=2,ns=2), "+/-",format(unitdmuErrors[[jjj]],digits=2, ns = 2)))
}
for(jjj in 1:length(unitList)){
    print(paste(unitList[[jjj]]," dA = ",format(unitdAAverages[[jjj]],digits=2,ns=2), "+/-",format(unitdAErrors[[jjj]],digits=2, ns = 2)))
}###################################################################################SOPHIA^^
