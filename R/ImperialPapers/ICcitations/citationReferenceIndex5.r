...
citationReferenceIndex5 <- function(rootName, numberbins=25, minCount=0, minYear=1900, maxYear=2010, normalise=TRUE, cNotCoverR=FALSE, numberParameters=3) {
...
if (cNotCoverR) nzcrlist<-citeList
...
#sqrt2pi <- sqrt(2*pi)
bigFactor=1e2
startlist <- list(sigma=sigmaMax, mu=log(nzpcr5n[3]), A=1)
lowerlist <- list(sigma=sigmaMax/bigFactor, mu=log(nzpcr5n[2]), A=1/bigFactor )
upperlist <- list(sigma=sigmaMax*bigFactor, mu=log(nzpcr5n[4]), A=bigFactor )
