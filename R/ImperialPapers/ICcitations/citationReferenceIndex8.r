...
citationReferenceIndex8 <- function(refList, citeList, numberbins=25, minCount=0, 
                                    normalise=TRUE, cNotCoverR=FALSE, numberParameters=3) {
...
lll <- length(clist)
binLower <-nzpcrhist$breaks[1:lll]
binUpper <-nzpcrhist$breaks[2:(lll+1)]
...
nzpcrdf <- data.frame(citeRefRatio = nzpcrhist$mids[clist>minCount], count = clist[clist>minCount], binWidth = binWidth[clist>minCount], binLower = binLower[clist>minCount], binUpper = binUpper[clist>minCount]);
#binWidth <-binWidthTemp[clist>minCount]
print("Bin count")
print(nzpcrdf$count)
print("Bin lower")
print(nzpcrdf$binLower)
...
#Now use explicit upper lower bounds in nzpcrdf

# Now use the difference of the cummulative disttibution (prob(x<=X)) to find expected number in bin given by mid values.
if (numberParameters==1) {
 fitres <- nls( count ~ totalPapers*(plnorm(binUpper, -sigma*sigma/2.0, sigma)-plnorm(binLower, -sigma*sigma/2.0, sigma) ) , nzpcrdf, startlist, lower=lowerlist, upper=upperlist, algorithm="port");
}
else {
 fitres <- nls( count ~ totalPapers*(1+dA)*(plnorm(binUpper, dmu-sigma*sigma/2, sigma)-plnorm(binLower, dmu-sigma*sigma/2, sigma) ) , nzpcrdf, startlist, lower=lowerlist, upper=upperlist, algorithm="port");
}
...
lst <-list(df=nzpcrdf, fit=fitres, totalPapers=totalPapers,  
           ...
           infoList=infoList, meanValueUnnorm= meanValueUnnorm )
