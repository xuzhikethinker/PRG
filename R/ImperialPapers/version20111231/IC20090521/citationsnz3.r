...
source("citationReferenceIndex2.r")
...
deptList <- titleString<-paste(rootName,"All") 
unitList <- list("All")
binSize <- seq(from=50, by=0, length=length(unitList))


# default settings use faculty
if (deptOn){
deptList <- titleString<-paste(rootName,"Faculties") 
...
titleString<-paste(rootName,"Departments Ranked",firstDept,"to",lastDept) 
...
CRList <- list()
for (iii in 1:length(unitList)) CRList[[iii]] <- citationReferenceIndex2(nameList[[iii]],binSize[[iii]], minCount) 
...
CRaverage[iii] <- infoList$totalNumberCitations/infoList$totalNumberReferences
...
windows()
plot(x=CRaverage,  y=fractionRetained, type="p", main=titleString, sub="<c>/<r> and fraction retained", xlab="<c>/<r>", ylab="fraction retained", pch=binType)

windows()
muCRdf <- data.frame(mu=muVector, CR=CRaverage)
muCRfit <- lm(mu ~ CR ,data=muCRdf)
summary(muCRfit)
plot(y=muVector,  x=CRaverage, type="p", main=paste(titleString,"mu and <c>/<r>"), ylab="mu", xlab="<c>/<r>", pch=binType)
text(y=muVector, x=CRaverage, labels=1:length(muVector), pos=4)
lines(fitted(muCRfit), x=CRaverage)



# Table of Names
windows()
barplot(CRaverage,names.arg=1:length(unitList))
...
pdfFileName<-paste(rootName,"all","ll.pdf",sep="_")
