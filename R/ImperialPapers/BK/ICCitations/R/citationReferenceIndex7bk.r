...
if (numberParameters==1) 
{
 ...
 upperlist <- list(sigma=sigmaMax*bigFactor)
  ...
  startlist <- list(sigma=sigmaMax,mu=log(nzpcr5n[3])/2)
 lowerlist <- list(sigma=sigmaMax/bigFactor,mu=log(nzpcr5n[3])/bigFactor)
 upperlist <- list(sigma=sigmaMax*bigFactor,mu=log(nzpcr5n[3])*bigFactor) 
 }
}
...
if (numberParameters==1) {
    if(normalise){
     ...
     fitres <- nls( count ~ totalPapers*(plnorm(citeRefRatio*midToUpperBinFactor, mu, sigma)-plnorm(citeRefRatio*midToLowerBinFactor, mu, sigma) ) , nzpcrdf, startlist, lower=lowerlist, upper=upperlist,algorithm="port");      
    }
}
else {
...
if(numberParameters==1){
    if(normalise){
        ...
        mu = coef(fitressum)[[2]]
        exp <- totalPapers*(plnorm(x*midToUpperBinFactor, mu-sigma*sigma/2.0, sigma)-plnorm(x*midToLowerBinFactor, mu-sigma*sigma/2.0, sigma) )     
    }
